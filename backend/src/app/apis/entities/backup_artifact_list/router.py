...
from app.apis.entities.backup_artifact_list.contract import CONTRACT
from app.apis.entities.backup_artifact_list.functions import execute
from app.apis.entities.backup_artifact_list.schemas import BackupArtifactRow
...
@router.get(
    ...
    response_model=list[BackupArtifactRow],
    responses={
...
def handle(
    ...
    """本人へ発行したバックアップの証拠。本文を保存せず、削除後も匿名化した発行記録を保持するの一覧。認証情報は依存から取得し、本人所有または管理者権限を検査する。"""
