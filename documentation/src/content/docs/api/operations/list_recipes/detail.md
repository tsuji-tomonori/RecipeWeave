...
| 関数 | 責務 | 定義元 |
|---|---|---|
| list_recipes | 個別の説明なし。下記の実装を参照。 | backend/src/app/apis/recipes/list_recipes/router.py:15 |
| has_matching_ingredients | 選択食材の全件一致・部分一致を適用する。人数は検索条件に含めない。 | backend/src/app/apis/recipes/list_recipes/functions.py:9 |
| has_excluded_food | 判明している構成食材を含む複合食品を除外する。アレルギー対応を保証しない。 | backend/src/app/apis/recipes/list_recipes/functions.py:18 |
| list_recipes | 件数を水増しせず、対象を限定したサンプル料理を検索する。 | backend/src/app/apis/recipes/list_recipes/functions.py:35 |
...
[インターフェース](/RecipeWeave/quality/design/api/operations/list_recipes/interface/) / [シーケンス](/RecipeWeave/quality/design/api/operations/list_recipes/sequence/) / [SQL](/RecipeWeave/quality/design/api/operations/list_recipes/queries/) / [検証](/RecipeWeave/quality/design/api/operations/list_recipes/tests/)
