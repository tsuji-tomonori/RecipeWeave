...
| `database/tables/<schema>.<table>.md` | pglastのPostgreSQL DDL AST | 列、型、NULL、既定値、主キー、一意制約、外部キー、CHECK、索引、出典 |
...
生成図は各操作のrouterと到達可能なfunctionsからポート呼出しまでを対象にする。分岐や繰返しを保持し、未対応構文はファイル・関数・行・構文種別を示して失敗する。依存性注入、Cognito認証、サービス先のPostgreSQL処理等を解析していなければ、その内部を完全に展開した図とは記載しない。
...
元スプレッドシート71表の取得snapshotと実DDLの対応を `database/schema_catalog.json` で検証する。運用追加表と移行台帳は元定義と区別し、全物理表の一覧・仕様・ERへ含める。各APIは独立OpenAPI、CRUD、SQL、シーケンス、処理詳細、試験対応を生成する。

CornellNoteWebv2の参照版と採用範囲は `docs/design/ADR-0002-relational-service.md` に記録する。実PostgreSQLの非管理者ロールによる制約・分離・操作試験と、PC・モバイルのGiven/When/Then、画面画像・trace・動画・JUnitを同じコミットへ対応させる。未実行・スキップ・失敗を成功として集計しない。品質ページの検索・図・画像拡大も実ブラウザで検証する。

Pagesはアプリ・品質・設計サイトを公開する。DB・Cognito・APIのAWS実配備は別の実行結果で確認し、静的公開だけで実配備完了としない。
