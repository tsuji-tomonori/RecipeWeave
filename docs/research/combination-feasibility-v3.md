...
各集合から単純無作為非復元抽出 400 件、各行を Luna の2評価枠で評価した記録を集計した。開発 pilot 100 件は v2 確認母集団から除外し、確認仮説の検定には使っていない。seed、定義、プロトコルは保存されているが、実行ID・prompt/context hash・実行時刻の証跡がなく、評価時点の条件固定とコンテキストの独立性を保存ファイルだけで再検証することはできない。
...
- 評価入力は版名・cohort列を除いているが、料理構造のラベル集合は旧版と改良版で共通しない。内容から評価群を推測できるため、完全盲検とは扱わない。期待による判定の偏りは未測定である。
- 実行モデル名は当時の作業記録に基づく申告である。集計処理はモデル実行を観測しておらず、`actual_model` を確認済み値として生成しない。過去の実行メタデータを後付けで作らない。
- 今回、保存済み確認設計とpilot設計のdigestを検査器へ固定し、以後の条件差替えと除外集合の不一致を拒否する。これは今回以後の変更検出であり、評価前の事前登録を証明するものではない。
- 確認的な検定は事前指定の「両者pass」の一回だけ。副指標のp値は多重性未調整の探索的参考値として明示し、棄却判定を付けない。主指標の件数・区間・p値は変更していない。
- 全量出力の全12,069,539行を生成定義と逐次照合した記録は [export-verification.json](../../experiments/confirmation/export-verification.json) に保存する。これは出力の整合性検査であり、料理の成立性や意味的な近似重複の証明ではない。
