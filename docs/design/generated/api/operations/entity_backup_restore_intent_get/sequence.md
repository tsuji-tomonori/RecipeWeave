# シーケンス: entity_backup_restore_intent_get
...
定義元: `backend/src/app/apis/entities/backup_restore_intent_get/router.py:38`
...
定義元: `backend/src/app/apis/entities/backup_restore_intent_get/functions.py:9`
...
sequenceDiagram
    ...
    Function->>Callee: service.execute(SPECIFICATIONS[#39;entity_backup_restore_intent_get#39;], row_id=row_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = service.execute(SPECIFICATIONS[#39;entity_backup_restore_intent_get#39;], row_id=row_id)
    Function->>Callee: BackupRestoreIntentRow.model_validate(rows[0])
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: BackupRestoreIntentRow.model_validate(rows[0])
    end
```
