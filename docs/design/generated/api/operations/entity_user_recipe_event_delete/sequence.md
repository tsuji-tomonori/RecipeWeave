# シーケンス: entity_user_recipe_event_delete
...
定義元: `backend/src/app/apis/entities/user_recipe_event_delete/router.py:32`
...
定義元: `backend/src/app/apis/entities/user_recipe_event_delete/functions.py:9`
...
sequenceDiagram
    ...
    Function->>Callee: service.execute(SPECIFICATIONS[#39;entity_user_recipe_event_delete#39;], row_id=row_id, if_match=if_match)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = service.execute(SPECIFICATIONS[#39;entity_user_recipe_event_delete#39;], row_id=row_id, if_match=if_match)
    Function->>Callee: UserRecipeEventRow.model_validate(rows[0])
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: UserRecipeEventRow.model_validate(rows[0])
    end
```
