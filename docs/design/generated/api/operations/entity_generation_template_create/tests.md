# 要因別単体テスト仕様: entity_generation_template_create
...
| 生成候補集合の成立性 | 未許可ID・重複・組の要素数とkの不一致 | 副材の組を検証 | 保存前に拒否して候補件数の水増しを防ぐ | backend/tests/test_entity_api.py::test_template_rejects_unsupported_or_duplicate_groups |
