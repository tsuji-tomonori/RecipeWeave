# 詳細設計: entity_generation_template_create
...
### `backend/src/app/apis/entities/generation_template_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.generation_template | C | id: 不変ID; created_at: 作成日時; code: テンプレートコード; version: 定義版; release_id: カタログ版; contract: 主副材の許可集合・k・味付・経路; candidate_count: この定義の正確な設計点数; contract_hash: 定義ハッシュ |

対象条件: `SQL上の絞り込みなし`

| SQLバインド | 実装上の値の出所 |
|---|---|
| candidate_count | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| code | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| contract | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| contract_hash | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/generation_template_create/router.py:31 |
| execute | GenerationTemplateRow.model_validate(rows[0]) | backend/src/app/apis/entities/generation_template_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 列挙テンプレート版の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/generation_template_create/router.py:31 |
| execute | 列挙テンプレート版の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/generation_template_create/functions.py:7 |
