# 詳細設計: entity_recipe_signature_create

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`POST /api/entities/recipe_signature` — 内容重複判定署名の作成
...
### `backend/src/app/apis/entities/recipe_signature_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_signature | C | id: 不変の行識別子; created_at: 作成日時（UTC）; recipe_version_id: 対象版; algorithm_version: 正規化アルゴリズム版; exact_hash: 材料比率・工程・主要条件のハッシュ; canonical_payload: 正規化対象の監査用内容; cluster_key: 料理近似群キー |
...
| canonical_payload | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| cluster_key | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| exact_hash | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_signature_create/router.py:31 |
| execute | RecipeSignatureRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_signature_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 内容重複判定署名の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_signature_create/router.py:31 |
| execute | 内容重複判定署名の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_signature_create/functions.py:7 |
