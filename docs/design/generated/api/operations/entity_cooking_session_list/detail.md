# 詳細設計: entity_cooking_session_list

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`GET /api/entities/cooking_session` — 調理計画実行の一覧
...
### `backend/src/app/apis/entities/cooking_session_list/sql/001_list.sql`
...
対象条件: `WHERE EXISTS(SELECT owner_0.id FROM recipeweave.menu AS owner_0 WHERE owner_0.id = t.menu_id AND owner_0.user_id = %(actor_id)s) AND (CAST(%(after_id)s AS UUID) IS NULL OR t.id > %(after_id)s)`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/cooking_session_list/router.py:31 |
| execute | [CookingSessionRow.model_validate(row) for row in rows] | backend/src/app/apis/entities/cooking_session_list/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 調理計画実行の一覧。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/cooking_session_list/router.py:31 |
| execute | 調理計画実行の一覧を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/cooking_session_list/functions.py:9 |
