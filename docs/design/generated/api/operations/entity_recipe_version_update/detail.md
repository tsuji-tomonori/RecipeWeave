# 詳細設計: entity_recipe_version_update
...
### `backend/src/app/apis/entities/recipe_version_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_version | U | id: 不変の行識別子; created_at: 作成日時（UTC）; recipe_id: 所属レシピ; version: 版番号; release_id: 採用カタログ版; base_servings: 登録分量が何人前か; output_amount: 完成量; output_unit_id: 完成量単位; status: 版の状態; validation: 公開審査; content_hash: 内容ハッシュ; published_at: 公開日時; description: 料理の紹介文 |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| base_servings | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| content_hash | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| description | '認証が必要' (backend/src/app/apis/entities/recipe_version_update/router.py:23) / '操作・参照権限なし' (backend/src/app/apis/entities/recipe_version_update/router.py:24) / '同時更新またはDB業務制約違反' (backend/src/app/apis/entities/recipe_version_update/router.py:25) / '入力不正' (backend/src/app/apis/entities/recipe_version_update/router.py:26) / 'If-Matchが必要' (backend/src/app/apis/entities/recipe_version_update/router.py:27) / 'DB接続不可' (backend/src/app/apis/entities/recipe_version_update/router.py:28) |
| expected_etag | parse_etag(if_match) (backend/src/app/core/entity_service.py:72) |
| output_amount | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| output_unit_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| published_at | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| recipe_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| release_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/recipe_version_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
| status | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| validation | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| version | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `recipe_id = %(recipe_id)s; version = %(version)s; release_id = %(release_id)s; base_servings = %(base_servings)s; output_amount = %(output_amount)s; output_unit_id = %(output_unit_id)s; status = %(status)s; validation = %(validation)s; content_hash = %(content_hash)s; published_at = %(published_at)s; description = %(description)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_version_update/router.py:32 |
| execute | RecipeVersionRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_version_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | レシピ内容版の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_version_update/router.py:32 |
| execute | レシピ内容版の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_version_update/functions.py:9 |
