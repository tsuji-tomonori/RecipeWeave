...
| SQLバインド | 実装上の値の出所 |
|---|---|
| item_id | item_id (backend/src/app/core/cooking_plan_service.py:82) / item_id (backend/src/app/core/cooking_plan_service.py:91) |
| position | position (backend/src/app/core/cooking_plan_service.py:82) |
| servings | item.servings (backend/src/app/core/cooking_plan_service.py:82) |
| version_id | _uuid(item.recipe_version_id) (backend/src/app/core/cooking_plan_service.py:68) / version_id (backend/src/app/core/cooking_plan_service.py:93) / version_id (backend/src/app/core/cooking_plan_service.py:82) / version_id (backend/src/app/core/cooking_plan_service.py:91) / version_id (backend/src/app/integrations/catalog/postgres_provider.py:62) |

代入・選択式: `CAST(%(item_id)s AS UUID) AS item_id; CAST(%(position)s AS INT) AS position; CAST(%(servings)s AS DECIMAL) AS servings; rv.base_servings; rv.recipe_id; st.id AS step_id; st.step_no; st.duration_max_s; st.attention; sc.mode AS scaling_mode; sc.batch_capacity; sc.min_servings; sc.max_servings`
...
| SQLバインド | 実装上の値の出所 |
|---|---|
| user_id | self.identity.user_id (backend/src/app/core/cooking_plan_service.py:95) |
...
| SQLバインド | 実装上の値の出所 |
|---|---|
| equipment | [resource_names[resource_id] for resource_id, _ in task.reservations] (backend/src/app/core/cooking_plan_service.py:116) / equipment or [] (backend/src/app/integrations/catalog/postgres_provider.py:48) |
...
| owner_id | self.identity.user_id (backend/src/app/core/cooking_plan_service.py:68) / owner_id (backend/src/app/integrations/catalog/postgres_provider.py:62) |
| preview | local_auth_enabled() (backend/src/app/core/cooking_plan_service.py:68) / preview (backend/src/app/integrations/catalog/postgres_provider.py:48) |
| q | query (backend/src/app/integrations/catalog/postgres_provider.py:48) |
| recipe_id | _uuid(item.recipe_id) (backend/src/app/core/cooking_plan_service.py:68) / recipe.id (backend/src/app/core/cooking_plan_service.py:111) / recipe_id (backend/src/app/integrations/catalog/postgres_provider.py:48) |
...
| 判定条件 | 例外・応答 | 定義元 |
|---|---|---|
| None in keys or set(item.amounts) != keys | HTTPException(422, '指定した料理版と材料の構成が一致しません') | backend/src/app/core/cooking_plan_service.py:36 |
| ingredient.ingredient_id is None | HTTPException(422, '材料行の識別子が登録されていません') | backend/src/app/core/cooking_plan_service.py:36 |
| amount.value is None or amount.unit != ingredient.quantity.unit | HTTPException(422, '材料の量を確定し、登録済みの単位で指定してください') | backend/src/app/core/cooking_plan_service.py:36 |
| item_id in recipes | HTTPException(422, '同じ献立行が重複しています') | backend/src/app/core/cooking_plan_service.py:56 |
| not recipe_rows | HTTPException(404, 'この料理版は利用できません') | backend/src/app/core/cooking_plan_service.py:56 |
...
| _uuid | UUID(value or '') | backend/src/app/core/cooking_plan_service.py:29 |
| CookingPlanService.preview | PlanResponse(plan=result) | backend/src/app/core/cooking_plan_service.py:56 |
...
| _uuid | 個別説明なし | backend/src/app/core/cooking_plan_service.py:29 |
| validate_item | 同一食品の複数材料行を混同せず、単位や未知量の不整合を拒否する。 | backend/src/app/core/cooking_plan_service.py:36 |
| CookingPlanService.preview | 最新の設備と閲覧可能な料理版から、変更を保存せず計算する。 | backend/src/app/core/cooking_plan_service.py:56 |
