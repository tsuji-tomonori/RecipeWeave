# 詳細設計: entity_shopping_item_delete

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`DELETE /api/entities/shopping_item/{row_id}` — 買い物行の削除
...
### `backend/src/app/apis/entities/shopping_item_delete/sql/001_delete.sql`
...
| recipeweave.shopping_item | D | id: 不変の行識別子; created_at: 作成日時（UTC）; session_id: 対象調理; total_id: 需要行; product_version_id: 購入SKU; net_shortage: 在庫控除後の不足量; package_count: 購入包装数; surplus_amount: 購入後余剰; checked: 購入済み; client_key: 画面操作の安定キー; checked_at: 購入確認日時; archived: 完了した買い物の保管状態 |
...
| row_id | row_id (backend/src/app/apis/entities/shopping_item_delete/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/shopping_item_delete/router.py:32 |
| execute | ShoppingItemRow.model_validate(rows[0]) | backend/src/app/apis/entities/shopping_item_delete/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 買い物行の削除。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/shopping_item_delete/router.py:32 |
| execute | 買い物行の削除を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/shopping_item_delete/functions.py:9 |
