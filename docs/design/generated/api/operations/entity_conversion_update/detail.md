# 詳細設計: entity_conversion_update

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PUT /api/entities/conversion/{row_id}` — 食材形態別換算の更新
...
### `backend/src/app/apis/entities/conversion_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.conversion | U | id: 不変の行識別子; created_at: 作成日時（UTC）; form_id: 換算対象形態; from_unit_id: 入力単位; to_unit_id: 出力単位; factor: 出力量=入力量×倍率; quality: 実測・推定区別; source_id: 換算根拠; conditions: サイズ・温度・すり切り等; release_id: 換算版 |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| conditions | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| expected_etag | parse_etag(if_match) (backend/src/app/core/entity_service.py:71) |
| factor | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| form_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| from_unit_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| quality | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| release_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/conversion_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
| source_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| to_unit_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `form_id = %(form_id)s; from_unit_id = %(from_unit_id)s; to_unit_id = %(to_unit_id)s; factor = %(factor)s; quality = %(quality)s; source_id = %(source_id)s; conditions = %(conditions)s; release_id = %(release_id)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/conversion_update/router.py:32 |
| execute | ConversionRow.model_validate(rows[0]) | backend/src/app/apis/entities/conversion_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 食材形態別換算の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/conversion_update/router.py:32 |
| execute | 食材形態別換算の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/conversion_update/functions.py:9 |
