# 詳細設計: entity_recipe_step_create

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`POST /api/entities/recipe_step` — 調理工程節点の作成
...
### `backend/src/app/apis/entities/recipe_step_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_step | C | id: 不変の行識別子; created_at: 作成日時（UTC）; recipe_version_id: 所属版; step_no: 表示順（依存順とは別）; operation_id: 標準動作; instruction: 個別補足; attention: 作業者拘束; duration_min_s: 所要秒下限; duration_max_s: 所要秒上限; scaling_rule_id: 時間の人数変更規則; completion_cue: 実測・目視の終了条件; title: 工程の短い見出し |
...
| operation_id | CONTRACT.operation_id (backend/src/app/apis/entities/recipe_step_create/router.py:14) / spec.operation_id (backend/src/app/core/entity_service.py:50) / spec.operation_id (backend/src/app/core/entity_service.py:109) / spec.operation_id (backend/src/app/core/entity_service.py:119) |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_step_create/router.py:28 |
| execute | RecipeStepRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_step_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 調理工程節点の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_step_create/router.py:28 |
| execute | 調理工程節点の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_step_create/functions.py:7 |
