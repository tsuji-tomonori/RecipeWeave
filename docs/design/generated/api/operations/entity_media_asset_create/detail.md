# 詳細設計: entity_media_asset_create

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`POST /api/entities/media_asset` — 教育用動画等の版の作成
...
### `backend/src/app/apis/entities/media_asset_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.media_asset | C | id: 不変の行識別子; created_at: 作成日時（UTC）; operation_id: 説明する標準動作; media_type: 動画/アニメ/画像; uri: オブジェクト格納先; sha256: 資産ハッシュ; locale: 字幕言語; version: 媒体版; parameter_contract: 対応厚み・食材形状・視点; source_id: 権利・作成根拠; validation: 内容検証 |

対象条件: `SQL上の絞り込みなし`

| SQLバインド | 実装上の値の出所 |
|---|---|
| locale | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| media_type | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| operation_id | CONTRACT.operation_id (backend/src/app/apis/entities/media_asset_create/router.py:14) / spec.operation_id (backend/src/app/core/entity_service.py:49) / spec.operation_id (backend/src/app/core/entity_service.py:108) / spec.operation_id (backend/src/app/core/entity_service.py:118) |
| parameter_contract | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| sha256 | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| source_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| uri | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/media_asset_create/router.py:28 |
| execute | MediaAssetRow.model_validate(rows[0]) | backend/src/app/apis/entities/media_asset_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 教育用動画等の版の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/media_asset_create/router.py:28 |
| execute | 教育用動画等の版の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/media_asset_create/functions.py:7 |
