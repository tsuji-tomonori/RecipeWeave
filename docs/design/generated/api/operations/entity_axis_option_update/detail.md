# 詳細設計: entity_axis_option_update
...
### `backend/src/app/apis/entities/axis_option_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.axis_option | U | id: 不変の行識別子; created_at: 作成日時（UTC）; axis_id: 親軸; code: 値コード; label: 候補名; definition: 値の意味; parent_id: 同軸の階層親; status: 選択可否 |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| axis_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| label | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| parent_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/axis_option_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
...
代入・選択式: `axis_id = %(axis_id)s; code = %(code)s; label = %(label)s; definition = %(definition)s; parent_id = %(parent_id)s; status = %(status)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/axis_option_update/router.py:32 |
| execute | AxisOptionRow.model_validate(rows[0]) | backend/src/app/apis/entities/axis_option_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 軸候補値の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/axis_option_update/router.py:32 |
| execute | 軸候補値の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/axis_option_update/functions.py:9 |
