# 詳細設計: entity_menu_delete

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`DELETE /api/entities/menu/{row_id}` — 献立の削除
...
### `backend/src/app/apis/entities/menu_delete/sql/001_delete.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.menu | D | id: 不変の行識別子; created_at: 作成日時（UTC）; user_id: 所有者; name: 献立名; servings: 標準人数; revision: 楽観ロック版 |
...
| row_id | row_id (backend/src/app/apis/entities/menu_delete/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/menu_delete/router.py:32 |
| execute | MenuRow.model_validate(rows[0]) | backend/src/app/apis/entities/menu_delete/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 献立の削除。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/menu_delete/router.py:32 |
| execute | 献立の削除を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/menu_delete/functions.py:9 |
