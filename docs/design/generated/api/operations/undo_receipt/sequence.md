...
定義元: `backend/src/app/apis/workspace/undo_receipt/router.py:24`
...
sequenceDiagram
    ...
    Caller->>Function: identity: IdentityDependency, database: DatabaseDependency, request: RevisionRequest, row_id: UUID
...
定義元: `backend/src/app/apis/workspace/undo_receipt/functions.py:8`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: service: WorkspaceService, request: RevisionRequest, row_id: UUID
    Function->>Callee: service.undo_receipt(request, row_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: service.undo_receipt(request, row_id)
    end
```
...
定義元: `backend/src/app/core/workspace_service.py:481`

```mermaid
sequenceDiagram
    participant Caller as 呼出元
    participant Function as undo_receipt
    participant Callee as 呼出先
    Caller->>Function: self, request: RevisionRequest, row_id: UUID
    Function->>Callee: self.begin(#39;undo_receipt#39;, request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: q = self.begin(#39;undo_receipt#39;, request)
    Function->>Callee: q.run(#39;q001_import#39;, row_id=row_id, user_id=self.user_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = q.run(#39;q001_import#39;, row_id=row_id, user_id=self.user_id)
    Note over Function: 条件付き式を評価: not rows or rows[0][#39;status#39;] != #39;committed#39;
    alt not rows or rows[0][#39;status#39;] != #39;committed#39;
        Function->>Callee: HTTPException(409, #39;レシートがないか取消済みです#39;)
        Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
        break この経路の関数終了: raise
            Function-->>Caller: HTTPException(409, #39;レシートがないか取消済みです#39;)
        end
    end
    Function->>Callee: q.run(#39;q002_eligible_lots#39;, row_id=row_id, user_id=self.user_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Function->>Callee: q.run(#39;q003_revert#39;, row_id=row_id, user_id=self.user_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
