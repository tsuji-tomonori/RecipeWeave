...
| not rows or rows[0]['status'] != 'committed' | HTTPException(409, 'レシートがないか取消済みです') | backend/src/app/core/workspace_service.py:481 |
...
| WorkspaceService.undo_receipt | self.finish(q) | backend/src/app/core/workspace_service.py:481 |
...
| WorkspaceService.undo_receipt | 未使用・未編集の登録分だけを取り消し、使用済みの在庫は残す。 | backend/src/app/core/workspace_service.py:481 |
