...
`POST /api/receipts/{row_id}/undo` — レシート登録を取り消す
...
### `backend/src/app/apis/workspace/undo_receipt/sql/q001_import.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.receipt_import | R | id: 不変の行識別子; user_id: 所有者; status: draft/committed/revertedの状態 |

対象条件: `WHERE id = %(row_id)s AND user_id = %(user_id)s`

行ロック: `FOR UPDATE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | uuid4() (backend/src/app/core/workspace_service.py:71) / row_id (backend/src/app/core/workspace_service.py:483) / row_id (backend/src/app/core/workspace_service.py:486) / row_id (backend/src/app/core/workspace_service.py:487) |
| user_id | self.user_id (backend/src/app/core/workspace_service.py:64) / self.user_id (backend/src/app/core/workspace_service.py:70) / self.user_id (backend/src/app/core/workspace_service.py:71) / self.user_id (backend/src/app/core/workspace_service.py:83) / self.user_id (backend/src/app/core/workspace_service.py:84) / self.user_id (backend/src/app/core/workspace_service.py:132) / self.user_id (backend/src/app/core/workspace_service.py:105) / self.user_id (backend/src/app/core/workspace_service.py:119) / self.user_id (backend/src/app/core/workspace_service.py:147) / self.user_id (backend/src/app/core/workspace_service.py:158) / self.user_id (backend/src/app/core/workspace_service.py:171) / self.user_id (backend/src/app/core/workspace_service.py:187) / self.user_id (backend/src/app/core/workspace_service.py:205) / self.user_id (backend/src/app/core/workspace_service.py:483) / self.user_id (backend/src/app/core/workspace_service.py:486) / self.user_id (backend/src/app/core/workspace_service.py:487) |

代入・選択式: `id; status`

### `backend/src/app/apis/workspace/undo_receipt/sql/q002_eligible_lots.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.pantry_consumption | R | lot_id: 消費元ロット |
| recipeweave.pantry_lot | U | id: 不変の行識別子; user_id: 所有者; status: 在庫の有効・削除・レシート取消状態; source_import_id: 登録元レシート; updated_at: 最終編集日時; edited: 登録後の編集有無 |

対象条件: `WHERE p.source_import_id = %(row_id)s AND p.user_id = %(user_id)s AND NOT p.edited AND NOT EXISTS(SELECT 1 FROM recipeweave.pantry_consumption AS c WHERE c.lot_id = p.id)`
...
代入・選択式: `status = 'undone'; updated_at = CURRENT_TIMESTAMP`

### `backend/src/app/apis/workspace/undo_receipt/sql/q003_revert.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.receipt_import | U | id: 不変の行識別子; user_id: 所有者; status: draft/committed/revertedの状態; revision: 楽観ロック版; reverted_at: 登録取消日時 |

対象条件: `WHERE id = %(row_id)s AND user_id = %(user_id)s AND status = 'committed'`
...
代入・選択式: `status = 'reverted'; reverted_at = CURRENT_TIMESTAMP; revision = revision + 1`

### `backend/src/app/apis/workspace/undo_receipt/sql/q900_lock_revision.sql`
...
### `backend/src/app/apis/workspace/undo_receipt/sql/q901_advance_revision.sql`
...
### `backend/src/app/apis/workspace/undo_receipt/sql/q902_append_audit.sql`
...
| not rows or rows[0]['status'] != 'committed' | HTTPException(409, 'レシートがないか取消済みです') | backend/src/app/core/workspace_service.py:480 |

## 出力

| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | execute(WorkspaceService(database, identity), request, row_id) | backend/src/app/apis/workspace/undo_receipt/router.py:24 |
| execute | service.undo_receipt(request, row_id) | backend/src/app/apis/workspace/undo_receipt/functions.py:8 |
...
| WorkspaceService.undo_receipt | self.finish(q) | backend/src/app/core/workspace_service.py:480 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | レシート登録を取り消す。呼出元が送った利用者IDは使用しない。 | backend/src/app/apis/workspace/undo_receipt/router.py:24 |
| execute | レシート登録を取り消す。永続値は業務サービスが検証し、同一トランザクションで扱う。 | backend/src/app/apis/workspace/undo_receipt/functions.py:8 |
...
| WorkspaceService.undo_receipt | 未使用・未編集の登録分だけを取り消し、使用済みの在庫は残す。 | backend/src/app/core/workspace_service.py:480 |
