# 詳細設計: entity_product_component_create
...
### `backend/src/app/apis/entities/product_component_create/sql/001_create.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/product_component_create/router.py:31 |
| execute | ProductComponentRow.model_validate(rows[0]) | backend/src/app/apis/entities/product_component_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | セット内構成品の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/product_component_create/router.py:31 |
| execute | セット内構成品の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/product_component_create/functions.py:7 |
