# 詳細設計: entity_validation_result_create

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`POST /api/entities/validation_result` — 公開前評価結果の作成
...
### `backend/src/app/apis/entities/validation_result_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.validation_result | C | id: 不変の行識別子; created_at: 作成日時（UTC）; recipe_version_id: 対象版; rule_id: 適用規則版; state: 結果; evidence: 検査箇所・値・根拠; validator_version: 検証器版; evaluated_at: 検査日時 |

対象条件: `SQL上の絞り込みなし`

| SQLバインド | 実装上の値の出所 |
|---|---|
| evaluated_at | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| evidence | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| rule_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| state | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| validator_version | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/validation_result_create/router.py:31 |
| execute | ValidationResultRow.model_validate(rows[0]) | backend/src/app/apis/entities/validation_result_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 公開前評価結果の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/validation_result_create/router.py:31 |
| execute | 公開前評価結果の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/validation_result_create/functions.py:7 |
