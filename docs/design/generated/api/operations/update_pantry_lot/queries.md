...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.pantry_lot | U | amount, edited, expires_on, form_id, id, location, priority, quantity_quality, source_import_id, status, unit_id, updated_at, user_id |
| recipeweave.receipt_import | R | id, status |

バインド変数: amount, expires_on, form_id, location, priority, quality, restore, row_id, unit_id, user_id
