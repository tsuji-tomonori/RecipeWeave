...
定義元: `backend/src/app/apis/workspace/update_pantry_lot/router.py:24`
...
sequenceDiagram
    ...
    Caller->>Function: identity: IdentityDependency, database: DatabaseDependency, request: UpdatePantryRequest, row_id: UUID
...
定義元: `backend/src/app/apis/workspace/update_pantry_lot/functions.py:8`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: service: WorkspaceService, request: UpdatePantryRequest, row_id: UUID
    Function->>Callee: service.update_pantry_lot(request, row_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: service.update_pantry_lot(request, row_id)
    end
```
...
### workspace_service.py: `update_pantry_lot`

定義元: `backend/src/app/core/workspace_service.py:246`

```mermaid
sequenceDiagram
    participant Caller as 呼出元
    participant Function as update_pantry_lot
    participant Callee as 呼出先
    Caller->>Function: self, request: UpdatePantryRequest, row_id: UUID
    Function->>Callee: self.begin(#39;update_pantry_lot#39;, request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: q = self.begin(#39;update_pantry_lot#39;, request)
    Function->>Callee: self._stock(q, request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Function->>Callee: q.run(#39;q002_update_lot#39;, **self._stock(q, request), row_id=row_id, restore=request.restore)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = q.run(#39;q002_update_lot#39;, **self._stock(q, request), row_id=row_id, restore=request.restore)
    alt not rows
        Function->>Callee: HTTPException(404, #39;この在庫は編集・復元できません#39;)
        Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
        break この経路の関数終了: raise
            Function-->>Caller: HTTPException(404, #39;この在庫は編集・復元できません#39;)
        end
    end
