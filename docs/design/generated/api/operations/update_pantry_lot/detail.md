# 詳細設計: update_pantry_lot

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PATCH /api/pantry-lots/{row_id}` — 手持ち食材を修正・復元する
...
### `backend/src/app/apis/workspace/update_pantry_lot/sql/q001_resolve_form.sql`
...
| SQLバインド | 実装上の値の出所 |
|---|---|
| food_id | identifier(value.food_id) (backend/src/app/core/workspace_service.py:210) |
| form | r['form'] (backend/src/app/core/workspace_service.py:93) / value.form (backend/src/app/core/workspace_service.py:210) |
| unit | unit (backend/src/app/core/workspace_service.py:43) / value.quantity.unit (backend/src/app/core/workspace_service.py:210) |
| user_id | self.user_id (backend/src/app/core/workspace_service.py:65) / self.user_id (backend/src/app/core/workspace_service.py:71) / self.user_id (backend/src/app/core/workspace_service.py:72) / self.user_id (backend/src/app/core/workspace_service.py:84) / self.user_id (backend/src/app/core/workspace_service.py:85) / self.user_id (backend/src/app/core/workspace_service.py:133) / self.user_id (backend/src/app/core/workspace_service.py:106) / self.user_id (backend/src/app/core/workspace_service.py:120) / self.user_id (backend/src/app/core/workspace_service.py:148) / self.user_id (backend/src/app/core/workspace_service.py:159) / self.user_id (backend/src/app/core/workspace_service.py:172) / self.user_id (backend/src/app/core/workspace_service.py:188) / self.user_id (backend/src/app/core/workspace_service.py:206) / self.user_id (backend/src/app/core/workspace_service.py:210) / self.user_id (backend/src/app/core/workspace_service.py:225) |

代入・選択式: `fm.id AS form_id; u.id AS unit_id`

### `backend/src/app/apis/workspace/update_pantry_lot/sql/q002_update_lot.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.pantry_lot | U | id: 不変の行識別子; user_id: 所有者; form_id: 食材形態; amount: 残量; unit_id: 単位; expires_on: 表示期限; location: 冷蔵・冷凍・常温の保管場所; priority: 先に使う優先指定; status: 在庫の有効・削除・レシート取消状態; source_import_id: 登録元レシート; quantity_quality: 数量の確定・不明; updated_at: 最終編集日時; edited: 登録後の編集有無 |
| recipeweave.receipt_import | R | id: 不変の行識別子; status: draft/committed/revertedの状態 |

対象条件: `WHERE p.id = %(row_id)s AND p.user_id = %(user_id)s AND (p.status = 'active' OR (p.status = 'deleted' AND %(restore)s)) AND NOT EXISTS(SELECT 1 FROM recipeweave.receipt_import AS r WHERE r.id = p.source_import_id AND r.status = 'reverted')`

| SQLバインド | 実装上の値の出所 |
|---|---|
| amount | Decimal(str(value.quantity.value)) if value.quantity.value is not None else None (backend/src/app/core/workspace_service.py:226) |
| expires_on | expires (backend/src/app/core/workspace_service.py:230) |
| form_id | 型付きクエリの引数。呼出元のSQL仕様を参照。 |
...
| restore | request.restore (backend/src/app/core/workspace_service.py:249) |
| row_id | uuid4() (backend/src/app/core/workspace_service.py:72) / row_id (backend/src/app/core/workspace_service.py:249) |
| unit_id | 型付きクエリの引数。呼出元のSQL仕様を参照。 |
| user_id | self.user_id (backend/src/app/core/workspace_service.py:65) / self.user_id (backend/src/app/core/workspace_service.py:71) / self.user_id (backend/src/app/core/workspace_service.py:72) / self.user_id (backend/src/app/core/workspace_service.py:84) / self.user_id (backend/src/app/core/workspace_service.py:85) / self.user_id (backend/src/app/core/workspace_service.py:133) / self.user_id (backend/src/app/core/workspace_service.py:106) / self.user_id (backend/src/app/core/workspace_service.py:120) / self.user_id (backend/src/app/core/workspace_service.py:148) / self.user_id (backend/src/app/core/workspace_service.py:159) / self.user_id (backend/src/app/core/workspace_service.py:172) / self.user_id (backend/src/app/core/workspace_service.py:188) / self.user_id (backend/src/app/core/workspace_service.py:206) / self.user_id (backend/src/app/core/workspace_service.py:210) / self.user_id (backend/src/app/core/workspace_service.py:225) |
...
代入・選択式: `form_id = %(form_id)s; amount = %(amount)s; unit_id = %(unit_id)s; expires_on = %(expires_on)s; location = %(location)s; priority = %(priority)s; quantity_quality = %(quality)s; status = 'active'; updated_at = CURRENT_TIMESTAMP; edited = TRUE`

### `backend/src/app/apis/workspace/update_pantry_lot/sql/q900_lock_revision.sql`
...
### `backend/src/app/apis/workspace/update_pantry_lot/sql/q901_advance_revision.sql`
...
### `backend/src/app/apis/workspace/update_pantry_lot/sql/q902_append_audit.sql`
...
| not rows | HTTPException(404, 'この在庫は編集・復元できません') | backend/src/app/core/workspace_service.py:246 |

## 出力

| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | execute(WorkspaceService(database, identity), request, row_id) | backend/src/app/apis/workspace/update_pantry_lot/router.py:24 |
| execute | service.update_pantry_lot(request, row_id) | backend/src/app/apis/workspace/update_pantry_lot/functions.py:8 |
...
| WorkspaceService.update_pantry_lot | self.finish(q) | backend/src/app/core/workspace_service.py:246 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 手持ち食材を修正・復元する。呼出元が送った利用者IDは使用しない。 | backend/src/app/apis/workspace/update_pantry_lot/router.py:24 |
| execute | 手持ち食材を修正・復元する。永続値は業務サービスが検証し、同一トランザクションで扱う。 | backend/src/app/apis/workspace/update_pantry_lot/functions.py:8 |
...
| WorkspaceService.update_pantry_lot | 元の登録値を保ち、編集・復元できる本人の在庫だけを更新する。 | backend/src/app/core/workspace_service.py:246 |
