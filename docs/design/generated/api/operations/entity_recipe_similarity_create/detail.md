# 詳細設計: entity_recipe_similarity_create

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`POST /api/entities/recipe_similarity` — 近似レシピ関係の作成
...
### `backend/src/app/apis/entities/recipe_similarity_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_similarity | C | id: 不変の行識別子; created_at: 作成日時（UTC）; left_version_id: 左版; right_version_id: 右版; algorithm_version: 評価器版; score: 類似度0..1; explanation: 材料/味付/工程の一致差分 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_similarity_create/router.py:31 |
| execute | RecipeSimilarityRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_similarity_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 近似レシピ関係の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_similarity_create/router.py:31 |
| execute | 近似レシピ関係の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_similarity_create/functions.py:7 |
