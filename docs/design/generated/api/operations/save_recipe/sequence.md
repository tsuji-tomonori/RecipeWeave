...
定義元: `backend/src/app/apis/workspace/save_recipe/router.py:24`
...
定義元: `backend/src/app/apis/workspace/save_recipe/functions.py:8`
...
sequenceDiagram
    ...
    Function->>Callee: service.save_recipe(request, row_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: service.save_recipe(request, row_id)
    end
```
...
定義元: `backend/src/app/core/workspace_service.py:342`

```mermaid
sequenceDiagram
    participant Caller as 呼出元
    participant Function as save_recipe
    participant Callee as 呼出先
    Caller->>Function: self, request: RevisionRequest, row_id: UUID
    Function->>Callee: self._save_recipe(#39;save_recipe#39;, request, row_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: self._save_recipe(#39;save_recipe#39;, request, row_id)
    end
```
