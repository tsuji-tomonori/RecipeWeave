...
| recipeweave.recipe_version | R | id, recipe_id, status, validation, version |

バインド変数: preview, recipe_id
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.user_recipe_event | C | id, kind, occurred_at, recipe_version_id, request_key, user_id |

バインド変数: request_key, row_id, user_id, version_id
