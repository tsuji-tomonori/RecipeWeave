...
定義元: `backend/src/app/apis/workspace/update_menu_item/router.py:24`
...
sequenceDiagram
    ...
    Caller->>Function: identity: IdentityDependency, database: DatabaseDependency, request: MenuItemRequest, row_id: UUID
...
定義元: `backend/src/app/apis/workspace/update_menu_item/functions.py:8`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: service: WorkspaceService, request: MenuItemRequest, row_id: UUID
    Function->>Callee: service.update_menu_item(request, row_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: service.update_menu_item(request, row_id)
    end
```
...
定義元: `backend/src/app/core/workspace_service.py:263`
...
sequenceDiagram
    ...
    Note over Function: 条件付き式を評価: q.run(#39;q010_recipe#39;, recipe_id=identifier(item.recipe_id), preview=catalog_preview_enabled(), requested_version_id=identifier(item.recipe_version_id) if item.recipe_version_id else None)
    Note over Function: version = q.run(#39;q010_recipe#39;, recipe_id=identifier(item.recipe_id), preview=catalog_preview_enabled(), requested_version_id=identifier(item.recipe_version_id) if item.recipe_version_id else None)
    alt not version
...
### workspace_service.py: `update_menu_item`

定義元: `backend/src/app/core/workspace_service.py:311`

```mermaid
sequenceDiagram
    participant Caller as 呼出元
    participant Function as update_menu_item
    participant Callee as 呼出先
    Caller->>Function: self, request: MenuItemRequest, row_id: UUID
    Function->>Callee: identifier(request.item.id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    alt identifier(request.item.id) != row_id
        Function->>Callee: HTTPException(422, #39;対象の料理IDが一致しません#39;)
        Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
        break この経路の関数終了: raise
            Function-->>Caller: HTTPException(422, #39;対象の料理IDが一致しません#39;)
        end
    end
    Function->>Callee: self.begin(#39;update_menu_item#39;, request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: q = self.begin(#39;update_menu_item#39;, request)
    Function->>Callee: q.run(#39;q001_delete_item#39;, row_id=row_id, menu_id=self.menu_id, user_id=self.user_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    alt not q.run(#39;q001_delete_item#39;, row_id=row_id, menu_id=self.menu_id, user_id=self.user_id)
        Function->>Callee: HTTPException(404, #39;献立の料理が見つかりません#39;)
        Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
        break この経路の関数終了: raise
            Function-->>Caller: HTTPException(404, #39;献立の料理が見つかりません#39;)
        end
    end
