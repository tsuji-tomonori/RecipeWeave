...
### `backend/src/app/apis/entities/user_food_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.user_food | U | id: 不変の行識別子; created_at: 作成日時（UTC）; user_id: 所有者; food_id: 独自食材 |
...
| row_id | row_id (backend/src/app/apis/entities/user_food_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
...
### `backend/src/app/apis/entities/user_food_update/sql/002_reference_user_id.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/user_food_update/router.py:32 |
| execute | UserFoodRow.model_validate(rows[0]) | backend/src/app/apis/entities/user_food_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 利用者が追加した独自食材の所有の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/user_food_update/router.py:32 |
| execute | 利用者が追加した独自食材の所有の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/user_food_update/functions.py:9 |
