# インターフェース: entity_pantry_consumption_get
...
"""調理による在庫消費の冪等台帳の取得。具体例は専用型と操作別受入テストに対応する。"""
...
```json
{
  "description": "調理による在庫消費の冪等台帳の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。",
  "operationId": "entity_pantry_consumption_get",
  "parameters": [
    {
      "in": "path",
      "name": "row_id",
