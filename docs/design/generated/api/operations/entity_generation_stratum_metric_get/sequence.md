# シーケンス: entity_generation_stratum_metric_get
...
定義元: `backend/src/app/apis/entities/generation_stratum_metric_get/router.py:31`
...
sequenceDiagram
    ...
    Caller->>Function: response: Response, identity: IdentityDependency, database: DatabaseDependency, row_id: UUID
...
定義元: `backend/src/app/apis/entities/generation_stratum_metric_get/functions.py:9`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: service: EntityService, row_id: UUID
    Function->>Callee: service.execute(SPECIFICATIONS[#39;entity_generation_stratum_metric_get#39;], row_id=row_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = service.execute(SPECIFICATIONS[#39;entity_generation_stratum_metric_get#39;], row_id=row_id)
