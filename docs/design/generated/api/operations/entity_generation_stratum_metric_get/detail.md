# 詳細設計: entity_generation_stratum_metric_get

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`GET /api/entities/generation_stratum_metric/{row_id}` — 採用率・飽和度の実測の取得
...
### `backend/src/app/apis/entities/generation_stratum_metric_get/sql/001_get.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.generation_stratum_metric | R | id: 不変ID; created_at: 作成日時; template_id: 対象テンプレート; window_start: 計測窓開始; window_end: 計測窓終了; attempted: 試行数; valid: 適合生成数; unique_count: 既存集合との差分数; publishable: 公開基準通過数; input_tokens: 入力トークン合計; output_tokens: 出力トークン合計; cost_amount: 同一通貨の費用; currency: JPY/USD等; stratum_key: 層の安定キー（料理構造×食品カテゴリ×入手性）。集計定義はテンプレート版に固定 |

対象条件: `WHERE t.id = %(row_id)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | row_id (backend/src/app/apis/entities/generation_stratum_metric_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |

代入・選択式: `t.id; t.created_at; t.template_id; t.window_start; t.window_end; t.attempted; t.valid; t.unique_count; t.publishable; t.input_tokens; t.output_tokens; t.cost_amount; t.currency; t.stratum_key; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/generation_stratum_metric_get/router.py:31 |
| execute | GenerationStratumMetricRow.model_validate(rows[0]) | backend/src/app/apis/entities/generation_stratum_metric_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 採用率・飽和度の実測の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/generation_stratum_metric_get/router.py:31 |
| execute | 採用率・飽和度の実測の取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/generation_stratum_metric_get/functions.py:9 |
