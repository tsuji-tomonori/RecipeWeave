# インターフェース: entity_generation_stratum_metric_get
...
"""採用率・飽和度の実測の取得。具体例は専用型と操作別受入テストに対応する。"""
...
```json
{
  "description": "採用率・飽和度の実測の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。",
  "operationId": "entity_generation_stratum_metric_get",
  "parameters": [
    {
      "in": "path",
      "name": "row_id",
