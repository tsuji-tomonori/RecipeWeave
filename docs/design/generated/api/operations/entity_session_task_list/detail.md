# 詳細設計: entity_session_task_list

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`GET /api/entities/session_task` — 展開済み工程の一覧
...
### `backend/src/app/apis/entities/session_task_list/sql/001_list.sql`
...
対象条件: `WHERE EXISTS(SELECT owner_0.id FROM recipeweave.cooking_session AS owner_0 WHERE owner_0.id = t.session_id AND EXISTS(SELECT owner_1.id FROM recipeweave.menu AS owner_1 WHERE owner_1.id = owner_0.menu_id AND owner_1.user_id = %(actor_id)s)) AND (CAST(%(after_id)s AS UUID) IS NULL OR t.id > %(after_id)s)`
...
| after_id | after (backend/src/app/core/entity_service.py:68) |
| page_limit | limit (backend/src/app/core/entity_service.py:67) |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/session_task_list/router.py:31 |
| execute | [SessionTaskRow.model_validate(row) for row in rows] | backend/src/app/apis/entities/session_task_list/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 展開済み工程の一覧。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/session_task_list/router.py:31 |
| execute | 展開済み工程の一覧を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/session_task_list/functions.py:9 |
