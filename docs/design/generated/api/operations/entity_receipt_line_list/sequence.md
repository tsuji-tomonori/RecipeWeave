# シーケンス: entity_receipt_line_list
...
定義元: `backend/src/app/apis/entities/receipt_line_list/router.py:31`
...
定義元: `backend/src/app/apis/entities/receipt_line_list/functions.py:9`
...
sequenceDiagram
    ...
    Function->>Callee: service.execute(SPECIFICATIONS[#39;entity_receipt_line_list#39;], limit=limit, after=after)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = service.execute(SPECIFICATIONS[#39;entity_receipt_line_list#39;], limit=limit, after=after)
    Note over Function: 条件付き式を評価: [ReceiptLineRow.model_validate(row) for row in rows]
    break この経路の関数終了: return
        Function-->>Caller: [ReceiptLineRow.model_validate(row) for row in rows]
    end
```
