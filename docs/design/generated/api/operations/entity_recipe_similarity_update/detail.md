# 詳細設計: entity_recipe_similarity_update
...
### `backend/src/app/apis/entities/recipe_similarity_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_similarity | U | id: 不変の行識別子; created_at: 作成日時（UTC）; left_version_id: 左版; right_version_id: 右版; algorithm_version: 評価器版; score: 類似度0..1; explanation: 材料/味付/工程の一致差分 |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| algorithm_version | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| expected_etag | parse_etag(if_match) (backend/src/app/core/entity_service.py:71) |
| explanation | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| left_version_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| right_version_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/recipe_similarity_update/functions.py:16) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
| score | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `left_version_id = %(left_version_id)s; right_version_id = %(right_version_id)s; algorithm_version = %(algorithm_version)s; score = %(score)s; explanation = %(explanation)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_similarity_update/router.py:35 |
| execute | RecipeSimilarityRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_similarity_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 近似レシピ関係の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_similarity_update/router.py:35 |
| execute | 近似レシピ関係の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_similarity_update/functions.py:9 |
