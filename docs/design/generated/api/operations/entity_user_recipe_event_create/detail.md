# 詳細設計: entity_user_recipe_event_create
...
### `backend/src/app/apis/entities/user_recipe_event_create/sql/001_create.sql`
...
| SQLバインド | 実装上の値の出所 |
|---|---|
| kind | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| occurred_at | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| recipe_version_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| request_key | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
### `backend/src/app/apis/entities/user_recipe_event_create/sql/002_reference_user_id.sql`
...
### `backend/src/app/apis/entities/user_recipe_event_create/sql/003_reference_recipe_version_id.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/user_recipe_event_create/router.py:31 |
| execute | UserRecipeEventRow.model_validate(rows[0]) | backend/src/app/apis/entities/user_recipe_event_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 提案・調理履歴の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/user_recipe_event_create/router.py:31 |
| execute | 提案・調理履歴の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/user_recipe_event_create/functions.py:7 |
