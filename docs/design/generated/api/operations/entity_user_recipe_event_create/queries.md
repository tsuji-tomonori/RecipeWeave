# SQL仕様: entity_user_recipe_event_create
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.user_recipe_event | C | created_at, id, kind, occurred_at, recipe_version_id, request_key, user_id, xmin |

バインド変数: kind, occurred_at, recipe_version_id, request_key, row_id, user_id
...
| recipeweave.recipe_version | R | id, recipe_id, status, validation |
| recipeweave.user_recipe_event | R | recipe_version_id, user_id |

バインド変数: actor_id, preview, reference_id
