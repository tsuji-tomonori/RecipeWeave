# 詳細設計: entity_menu_ingredient_override_get
...
### `backend/src/app/apis/entities/menu_ingredient_override_get/sql/001_get.sql`
...
対象条件: `WHERE t.id = %(row_id)s AND EXISTS(SELECT owner_0.id FROM recipeweave.menu_item AS owner_0 WHERE owner_0.id = t.menu_item_id AND EXISTS(SELECT owner_1.id FROM recipeweave.menu AS owner_1 WHERE owner_1.id = owner_0.menu_id AND owner_1.user_id = %(actor_id)s))`
...
| row_id | row_id (backend/src/app/apis/entities/menu_ingredient_override_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |

代入・選択式: `t.id; t.created_at; t.menu_item_id; t.ingredient_line_id; t.selected; t.amount; t.form_id; t.product_version_id; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/menu_ingredient_override_get/router.py:31 |
| execute | MenuIngredientOverrideRow.model_validate(rows[0]) | backend/src/app/apis/entities/menu_ingredient_override_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 献立別材料確定の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/menu_ingredient_override_get/router.py:31 |
| execute | 献立別材料確定の取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/menu_ingredient_override_get/functions.py:9 |
