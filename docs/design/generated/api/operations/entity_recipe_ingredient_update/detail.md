# 詳細設計: entity_recipe_ingredient_update

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PUT /api/entities/recipe_ingredient/{row_id}` — レシピ材料明細の更新
...
### `backend/src/app/apis/entities/recipe_ingredient_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_ingredient | U | id: 不変の行識別子; created_at: 作成日時（UTC）; recipe_version_id: 親版; line_no: 表示順; form_id: 使用形態; product_version_id: 商品指定時の仕様版; component_id: セット内構成品を使う場合; kit_parent_line_id: 購入対象となるセットの親行; role: 料理での役割; demand_kind: 購入対象区分; amount_mode: 確定/範囲/適量; amount: 確定値または範囲下限; amount_max: 範囲上限; unit_id: 登録単位; canonical_amount: 登録版の基準量; conversion_id: 非基準単位の換算根拠; scaling_rule_id: 人数変換規則; optional: 任意追加材料; note: 材料の補足 |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| amount | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| amount_max | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| amount_mode | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| canonical_amount | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| component_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| conversion_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| demand_kind | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| kit_parent_line_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| line_no | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| note | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| optional | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| role | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/recipe_ingredient_update/functions.py:16) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
| scaling_rule_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `recipe_version_id = %(recipe_version_id)s; line_no = %(line_no)s; form_id = %(form_id)s; product_version_id = %(product_version_id)s; component_id = %(component_id)s; kit_parent_line_id = %(kit_parent_line_id)s; role = %(role)s; demand_kind = %(demand_kind)s; amount_mode = %(amount_mode)s; amount = %(amount)s; amount_max = %(amount_max)s; unit_id = %(unit_id)s; canonical_amount = %(canonical_amount)s; conversion_id = %(conversion_id)s; scaling_rule_id = %(scaling_rule_id)s; optional = %(optional)s; note = %(note)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_ingredient_update/router.py:35 |
| execute | RecipeIngredientRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_ingredient_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | レシピ材料明細の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_ingredient_update/router.py:35 |
| execute | レシピ材料明細の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_ingredient_update/functions.py:9 |
