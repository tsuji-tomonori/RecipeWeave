# 詳細設計: entity_operation_parameter_get

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`GET /api/entities/operation_parameter/{row_id}` — 動作パラメータ定義の取得
...
### `backend/src/app/apis/entities/operation_parameter_get/sql/001_get.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.operation_parameter | R | id: 不変の行識別子; created_at: 作成日時（UTC）; operation_id: 動作; code: thickness_mm等; name: 厚さ等; value_type: 値型; unit_id: 単位; required: 必須か; min_value: 許容下限; max_value: 許容上限; allowed_values: option型の具体値配列 |

対象条件: `WHERE t.id = %(row_id)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | row_id (backend/src/app/apis/entities/operation_parameter_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |

代入・選択式: `t.id; t.created_at; t.operation_id; t.code; t.name; t.value_type; t.unit_id; t.required; t.min_value; t.max_value; t.allowed_values; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/operation_parameter_get/router.py:31 |
| execute | OperationParameterRow.model_validate(rows[0]) | backend/src/app/apis/entities/operation_parameter_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 動作パラメータ定義の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/operation_parameter_get/router.py:31 |
| execute | 動作パラメータ定義の取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/operation_parameter_get/functions.py:9 |
