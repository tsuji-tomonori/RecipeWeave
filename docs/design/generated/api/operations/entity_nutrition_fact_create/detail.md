# 詳細設計: entity_nutrition_fact_create
...
### `backend/src/app/apis/entities/nutrition_fact_create/sql/001_create.sql`
...
| basis_amount | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| basis_unit_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| form_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| nutrient_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/nutrition_fact_create/router.py:28 |
| execute | NutritionFactRow.model_validate(rows[0]) | backend/src/app/apis/entities/nutrition_fact_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 形態・商品別栄養値の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/nutrition_fact_create/router.py:28 |
| execute | 形態・商品別栄養値の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/nutrition_fact_create/functions.py:7 |
