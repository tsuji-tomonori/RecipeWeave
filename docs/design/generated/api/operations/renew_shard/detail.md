...
`PUT /api/generation/shards/{row_id}/lease` — 生成リースの延長
...
### `backend/src/app/apis/generation/renew_shard/sql/001_execute.sql`
...
対象条件: `WHERE s.id = %(row_id)s AND s.lease_owner = %(lease_owner)s AND s.fence_token = %(expected_fence)s AND s.lease_expires_at > CURRENT_TIMESTAMP AND s.state = 'running'`
...
| row_id | row_id (backend/src/app/apis/generation/renew_shard/functions.py:14) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
...
代入・選択式: `lease_expires_at = CURRENT_TIMESTAMP + MAKE_INTERVAL(secs => %(lease_seconds)s)`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | execute(payload, EntityService(database, identity), row_id) | backend/src/app/apis/generation/renew_shard/router.py:31 |
| execute | run_lease_operation(service, 'renew_shard', values) | backend/src/app/apis/generation/renew_shard/functions.py:11 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 個別説明なし | backend/src/app/apis/generation/renew_shard/router.py:31 |
| execute | 生成リースの延長の値を検証済み主体と固定SQLへ渡す。 | backend/src/app/apis/generation/renew_shard/functions.py:11 |
