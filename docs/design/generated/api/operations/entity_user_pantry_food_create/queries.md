# SQL仕様: entity_user_pantry_food_create
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.user_pantry_food | C | created_at, food_id, id, user_id, xmin |

バインド変数: food_id, row_id, user_id

```sql
-- 利用者が常備すると設定した食材を作成する。
-- 値は名前付きパラメータで束縛する。
INSERT INTO recipeweave.user_pantry_food AS t (
    id,
    user_id,
    food_id
)
