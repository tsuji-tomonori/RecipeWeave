...
定義元: `backend/src/app/apis/recipes/get_recipe/router.py:32`
...
sequenceDiagram
    ...
    Caller->>Function: database: DatabaseDependency, catalog: CatalogDependency, recipe_id: UUID, credentials: Annotated[HTTPAuthorizationCredentials | None, Depends(bearer)], preview: Annotated[bool, Query()]=False, version_id: Annotated[UUID | None, Query(alias=#39;versionId#39;)]=None
    Function->>Callee: authorize_preview(preview, credentials, database)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: identity = authorize_preview(preview, credentials, database)
    Note over Function: 条件付き式を評価: identity is None and credentials is not None
    alt identity is None and credentials is not None
        Function->>Callee: require_identity(credentials, database)
        Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
        Note over Function: identity = require_identity(credentials, database)
    end
    Note over Function: 条件付き式を評価: api_functions.get_recipe(catalog, recipe_id, preview, version_id, identity.user_id if identity else None)
    Note over Function: recipe = api_functions.get_recipe(catalog, recipe_id, preview, version_id, identity.user_id if identity else None)
    alt recipe is None
        Function->>Callee: HTTPException(status_code=404, detail=#39;料理が見つかりません#39;)
        Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
        break この経路の関数終了: raise
            Function-->>Caller: HTTPException(status_code=404, detail=#39;料理が見つかりません#39;)
        end
    end
...
定義元: `backend/src/app/apis/recipes/get_recipe/functions.py:7`
...
sequenceDiagram
    ...
    participant Function as get_recipe
    participant Callee as 呼出先
    Caller->>Function: catalog: CatalogPort, recipe_id: UUID, preview: bool=False, version_id: UUID | None=None, owner_id: UUID | None=None
    Function->>Callee: catalog.recipes(operation=#39;get_recipe#39;, recipe_id=recipe_id, preview=preview, limit=1, version_id=version_id, owner_id=owner_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: items, _ = catalog.recipes(operation=#39;get_recipe#39;, recipe_id=recipe_id, preview=preview, limit=1, version_id=version_id, owner_id=owner_id)
    Note over Function: 条件付き式を評価: items[0] if items else None
    break この経路の関数終了: return
        Function-->>Caller: items[0] if items else None
    end
```
