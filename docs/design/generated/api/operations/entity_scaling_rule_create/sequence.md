# シーケンス: entity_scaling_rule_create
...
定義元: `backend/src/app/apis/entities/scaling_rule_create/router.py:28`
...
sequenceDiagram
    ...
    Caller->>Function: response: Response, identity: IdentityDependency, database: DatabaseDependency, payload: ScalingRuleWrite
...
定義元: `backend/src/app/apis/entities/scaling_rule_create/functions.py:7`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: service: EntityService, payload: ScalingRuleWrite
    Function->>Callee: payload.model_dump(mode=#39;python#39;, by_alias=True)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Function->>Callee: service.execute(SPECIFICATIONS[#39;entity_scaling_rule_create#39;], payload=payload.model_dump(mode=#39;python#39;, by_alias=True))
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = service.execute(SPECIFICATIONS[#39;entity_scaling_rule_create#39;], payload=payload.model_dump(mode=#39;python#39;, by_alias=True))
    Function->>Callee: ScalingRuleRow.model_validate(rows[0])
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: ScalingRuleRow.model_validate(rows[0])
    end
```
