# 詳細設計: entity_scaling_rule_create
...
### `backend/src/app/apis/entities/scaling_rule_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.scaling_rule | C | id: 不変の行識別子; created_at: 作成日時（UTC）; name: 規則名; mode: 比例・バッチ等; min_servings: 検証済み人数下限; max_servings: 検証済み人数上限; batch_capacity: 1バッチ上限; round_mode: 表示丸め; round_increment: 表示・購入の刻み; source_id: 検証根拠 |

対象条件: `SQL上の絞り込みなし`

| SQLバインド | 実装上の値の出所 |
|---|---|
| batch_capacity | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| max_servings | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| min_servings | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| mode | 'python' (backend/src/app/apis/entities/scaling_rule_create/functions.py:11) |
| name | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| round_increment | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| round_mode | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/scaling_rule_create/router.py:28 |
| execute | ScalingRuleRow.model_validate(rows[0]) | backend/src/app/apis/entities/scaling_rule_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 人数変更規則の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/scaling_rule_create/router.py:28 |
| execute | 人数変更規則の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/scaling_rule_create/functions.py:7 |
