# 詳細設計: entity_generation_job_get
...
### `backend/src/app/apis/entities/generation_job_get/sql/001_get.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.generation_job | R | id: 不変の行識別子; created_at: 作成日時（UTC）; policy_id: 実行方針; idempotency_key: 入力と方針から作る重複キー; status: 進行状態; started_at: 開始; finished_at: 終了; seed: 再現用seed; error_code: 失敗分類; attempt_count: 試行回数 |

対象条件: `WHERE t.id = %(row_id)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | row_id (backend/src/app/apis/entities/generation_job_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |

代入・選択式: `t.id; t.created_at; t.policy_id; t.idempotency_key; t.status; t.started_at; t.finished_at; t.seed; t.error_code; t.attempt_count; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/generation_job_get/router.py:31 |
| execute | GenerationJobRow.model_validate(rows[0]) | backend/src/app/apis/entities/generation_job_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 事前生成ジョブの取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/generation_job_get/router.py:31 |
| execute | 事前生成ジョブの取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/generation_job_get/functions.py:9 |
