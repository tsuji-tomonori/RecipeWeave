...
定義元: `backend/src/app/apis/workspace/create_pantry_lot/router.py:22`
...
sequenceDiagram
    ...
    Caller->>Function: identity: IdentityDependency, database: DatabaseDependency, request: CreatePantryRequest
...
定義元: `backend/src/app/apis/workspace/create_pantry_lot/functions.py:6`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: service: WorkspaceService, request: CreatePantryRequest
    Function->>Callee: service.create_pantry_lot(request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: service.create_pantry_lot(request)
    end
```
...
### workspace_service.py: `create_pantry_lot`

定義元: `backend/src/app/core/workspace_service.py:234`

```mermaid
sequenceDiagram
    participant Caller as 呼出元
    participant Function as create_pantry_lot
    participant Callee as 呼出先
    Caller->>Function: self, request: CreatePantryRequest
    Function->>Callee: self.begin(#39;create_pantry_lot#39;, request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: q = self.begin(#39;create_pantry_lot#39;, request)
    Function->>Callee: self._stock(q, request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Function->>Callee: identifier(request.id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Function->>Callee: q.run(#39;q002_insert_lot#39;, **self._stock(q, request), row_id=identifier(request.id), import_id=None)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
