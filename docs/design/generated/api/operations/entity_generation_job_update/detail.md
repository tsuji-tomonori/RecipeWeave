# 詳細設計: entity_generation_job_update
...
### `backend/src/app/apis/entities/generation_job_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.generation_job | U | id: 不変の行識別子; created_at: 作成日時（UTC）; policy_id: 実行方針; idempotency_key: 入力と方針から作る重複キー; status: 進行状態; started_at: 開始; finished_at: 終了; seed: 再現用seed; error_code: 失敗分類; attempt_count: 試行回数 |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| attempt_count | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| error_code | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| expected_etag | parse_etag(if_match) (backend/src/app/core/entity_service.py:72) |
| finished_at | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| idempotency_key | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| policy_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/generation_job_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
| seed | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| started_at | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `policy_id = %(policy_id)s; idempotency_key = %(idempotency_key)s; status = %(status)s; started_at = %(started_at)s; finished_at = %(finished_at)s; seed = %(seed)s; error_code = %(error_code)s; attempt_count = %(attempt_count)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/generation_job_update/router.py:32 |
| execute | GenerationJobRow.model_validate(rows[0]) | backend/src/app/apis/entities/generation_job_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 事前生成ジョブの更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/generation_job_update/router.py:32 |
| execute | 事前生成ジョブの更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/generation_job_update/functions.py:9 |
