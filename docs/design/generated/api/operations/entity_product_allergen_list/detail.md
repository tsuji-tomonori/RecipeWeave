# 詳細設計: entity_product_allergen_list

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`GET /api/entities/product_allergen` — 商品表示アレルゲンの一覧
...
### `backend/src/app/apis/entities/product_allergen_list/sql/001_list.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/product_allergen_list/router.py:31 |
| execute | [ProductAllergenRow.model_validate(row) for row in rows] | backend/src/app/apis/entities/product_allergen_list/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 商品表示アレルゲンの一覧。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/product_allergen_list/router.py:31 |
| execute | 商品表示アレルゲンの一覧を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/product_allergen_list/functions.py:9 |
