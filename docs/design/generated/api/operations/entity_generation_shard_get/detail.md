# 詳細設計: entity_generation_shard_get

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`GET /api/entities/generation_shard/{row_id}` — 列挙範囲・リース管理の取得
...
### `backend/src/app/apis/entities/generation_shard_get/sql/001_get.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.generation_shard | R | id: 不変ID; created_at: 作成日時; template_id: テンプレート版; start_ordinal: 開始序数; end_ordinal: 終了序数（排他的）; next_ordinal: 再開位置; lease_owner: ワーカー識別子; lease_expires_at: 有効期限; fence_token: 古い所有者の書込みを拒否; state: 待機/実行/完了/停止 |

対象条件: `WHERE t.id = %(row_id)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | row_id (backend/src/app/apis/entities/generation_shard_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |

代入・選択式: `t.id; t.created_at; t.template_id; t.start_ordinal; t.end_ordinal; t.next_ordinal; t.lease_owner; t.lease_expires_at; t.fence_token; t.state; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/generation_shard_get/router.py:31 |
| execute | GenerationShardRow.model_validate(rows[0]) | backend/src/app/apis/entities/generation_shard_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 列挙範囲・リース管理の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/generation_shard_get/router.py:31 |
| execute | 列挙範囲・リース管理の取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/generation_shard_get/functions.py:9 |
