...
定義元: `backend/src/app/apis/auth/local_login/router.py:17`
...
sequenceDiagram
    ...
    Caller->>Function: request: LoginRequest
    Function->>Callee: execute(request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: execute(request)
    end
```

### functions.py: `execute`

定義元: `backend/src/app/apis/auth/local_login/functions.py:4`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: request: LoginRequest
    Function->>Callee: local_login(request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: local_login(request)
    end
```
