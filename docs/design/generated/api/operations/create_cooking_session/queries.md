...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.menu_item | R | id, menu_id, position, recipe_version_id |
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.menu_ingredient_override | R | amount, ingredient_line_id, menu_item_id, selected |
| recipeweave.menu_item | R | id, menu_id, position, recipe_version_id, servings |
| recipeweave.recipe_ingredient | R | amount, conversion_id, demand_kind, form_id, id, line_no, optional, product_version_id, recipe_version_id, unit_id |
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.cooking_session | C | id, input_hash, input_snapshot, menu_id, menu_revision, planner_version, status, target_at |

バインド変数: hash, menu_id, revision, session_id, snapshot
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.task_dependency | C | after_task_id, before_task_id, id, max_lag_s, min_lag_s, reason |

バインド変数: after_id, before_id, max_lag, min_lag, reason, row_id
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.resource_reservation | C | end_s, id, quantity, resource_id, start_s, task_id |

バインド変数: end, quantity, resource_id, row_id, start, task_id
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.ingredient_total | C | calculation_version, form_id, id, product_version_id, quality, required_amount, session_id, unit_id |

バインド変数: amount, form_id, product_id, row_id, session_id, unit_id
