# 詳細設計: entity_step_dependency_update

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PUT /api/entities/step_dependency/{row_id}` — 工程依存辺の更新
...
### `backend/src/app/apis/entities/step_dependency_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.step_dependency | U | id: 不変の行識別子; created_at: 作成日時（UTC）; before_step_id: 先行工程; after_step_id: 後続工程; kind: 依存理由; min_lag_s: 完了後最低待機; max_lag_s: 品質上の最大待機 |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| after_step_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| before_step_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| row_id | row_id (backend/src/app/apis/entities/step_dependency_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
...
代入・選択式: `before_step_id = %(before_step_id)s; after_step_id = %(after_step_id)s; kind = %(kind)s; min_lag_s = %(min_lag_s)s; max_lag_s = %(max_lag_s)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/step_dependency_update/router.py:32 |
| execute | StepDependencyRow.model_validate(rows[0]) | backend/src/app/apis/entities/step_dependency_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 工程依存辺の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/step_dependency_update/router.py:32 |
| execute | 工程依存辺の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/step_dependency_update/functions.py:9 |
