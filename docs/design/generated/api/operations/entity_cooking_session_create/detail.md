# 詳細設計: entity_cooking_session_create

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`POST /api/entities/cooking_session` — 調理計画実行の作成
...
### `backend/src/app/apis/entities/cooking_session_create/sql/001_create.sql`
...
### `backend/src/app/apis/entities/cooking_session_create/sql/002_reference_menu_id.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/cooking_session_create/router.py:28 |
| execute | CookingSessionRow.model_validate(rows[0]) | backend/src/app/apis/entities/cooking_session_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 調理計画実行の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/cooking_session_create/router.py:28 |
| execute | 調理計画実行の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/cooking_session_create/functions.py:7 |
