# 詳細設計: entity_recipe_embedding_update
...
### `backend/src/app/apis/entities/recipe_embedding_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_embedding | U | id: 不変ID; created_at: 作成日時; recipe_version_id: 対象版; model_version: 埋め込みモデル固定版; content_hash: 入力内容ハッシュ; embedding: 仮定768次元float32; created_for_index: 検索索引版 |
...
| created_for_index | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| embedding | json.loads(row['embedding']) (backend/src/app/core/entity_service.py:100) |
| expected_etag | parse_etag(if_match) (backend/src/app/core/entity_service.py:72) |
| model_version | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| recipe_version_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/recipe_embedding_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
...
代入・選択式: `recipe_version_id = %(recipe_version_id)s; model_version = %(model_version)s; content_hash = %(content_hash)s; embedding = CAST(%(embedding)s AS VECTOR); created_for_index = %(created_for_index)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_embedding_update/router.py:35 |
| execute | RecipeEmbeddingRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_embedding_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 近似検索用特徴量の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_embedding_update/router.py:35 |
| execute | 近似検索用特徴量の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_embedding_update/functions.py:9 |
