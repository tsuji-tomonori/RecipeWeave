# 詳細設計: entity_task_dependency_update
...
### `backend/src/app/apis/entities/task_dependency_update/sql/001_update.sql`
...
| recipeweave.task_dependency | U | id: 不変の行識別子; created_at: 作成日時（UTC）; before_task_id: 先行タスク; after_task_id: 後続タスク; min_lag_s: 最小間隔; max_lag_s: 最大間隔; reason: 元DAG/洗浄/設備切替等 |
...
| after_task_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| before_task_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| expected_etag | parse_etag(if_match) (backend/src/app/core/entity_service.py:71) |
| max_lag_s | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| min_lag_s | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| reason | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/task_dependency_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
...
代入・選択式: `before_task_id = %(before_task_id)s; after_task_id = %(after_task_id)s; min_lag_s = %(min_lag_s)s; max_lag_s = %(max_lag_s)s; reason = %(reason)s`

### `backend/src/app/apis/entities/task_dependency_update/sql/002_reference_before_task_id.sql`
...
### `backend/src/app/apis/entities/task_dependency_update/sql/003_reference_after_task_id.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/task_dependency_update/router.py:32 |
| execute | TaskDependencyRow.model_validate(rows[0]) | backend/src/app/apis/entities/task_dependency_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 献立展開後依存の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/task_dependency_update/router.py:32 |
| execute | 献立展開後依存の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/task_dependency_update/functions.py:9 |
