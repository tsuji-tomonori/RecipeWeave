# SQL仕様: entity_user_pantry_food_update
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.user_pantry_food | U | created_at, food_id, id, user_id, xmin |

バインド変数: actor_id, expected_etag, food_id, row_id, user_id

```sql
-- 利用者が常備すると設定した食材を条件付き更新する。
-- 値は名前付きパラメータで束縛する。
UPDATE recipeweave.user_pantry_food AS t
SET
    user_id = %(user_id)s,
    food_id = %(food_id)s
WHERE
