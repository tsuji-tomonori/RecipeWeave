# SQL仕様: entity_receipt_line_delete
...
| recipeweave.receipt_line | D | amount, created_at, decision, form_id, id, import_id, line_no, pantry_lot_id, product_version_id, raw_name, unit_id, xmin |

バインド変数: actor_id, expected_etag, row_id

```sql
-- レシートの商品候補と確定した在庫の対応を条件付き削除する。
-- 値は名前付きパラメータで束縛する。
