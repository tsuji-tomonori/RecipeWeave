# 詳細設計: entity_recipe_version_create

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`POST /api/entities/recipe_version` — レシピ内容版の作成
...
### `backend/src/app/apis/entities/recipe_version_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_version | C | id: 不変の行識別子; created_at: 作成日時（UTC）; recipe_id: 所属レシピ; version: 版番号; release_id: 採用カタログ版; base_servings: 登録分量が何人前か; output_amount: 完成量; output_unit_id: 完成量単位; status: 版の状態; validation: 公開審査; content_hash: 内容ハッシュ; published_at: 公開日時; description: 料理の紹介文 |
...
| description | '認証が必要' (backend/src/app/apis/entities/recipe_version_create/router.py:20) / '操作・参照権限なし' (backend/src/app/apis/entities/recipe_version_create/router.py:21) / '同時更新またはDB業務制約違反' (backend/src/app/apis/entities/recipe_version_create/router.py:22) / '入力不正' (backend/src/app/apis/entities/recipe_version_create/router.py:23) / 'DB接続不可' (backend/src/app/apis/entities/recipe_version_create/router.py:24) |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_version_create/router.py:28 |
| execute | RecipeVersionRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_version_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | レシピ内容版の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_version_create/router.py:28 |
| execute | レシピ内容版の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_version_create/functions.py:7 |
