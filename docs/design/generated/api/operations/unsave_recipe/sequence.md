...
定義元: `backend/src/app/apis/workspace/unsave_recipe/router.py:24`
...
定義元: `backend/src/app/apis/workspace/unsave_recipe/functions.py:8`
...
sequenceDiagram
    ...
    Function->>Callee: service.unsave_recipe(request, row_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: service.unsave_recipe(request, row_id)
    end
```
...
定義元: `backend/src/app/core/workspace_service.py:328`

```mermaid
sequenceDiagram
    participant Caller as 呼出元
    participant Function as _save_recipe
    participant Callee as 呼出先
    Caller->>Function: self, name: str, request: RevisionRequest, row_id: UUID
    Function->>Callee: self.begin(name, request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: q = self.begin(name, request)
    Function->>Callee: catalog_preview_enabled()
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Function->>Callee: q.run(#39;q001_recipe#39;, recipe_id=row_id, preview=catalog_preview_enabled())
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = q.run(#39;q001_recipe#39;, recipe_id=row_id, preview=catalog_preview_enabled())
    alt not rows
...
### workspace_service.py: `unsave_recipe`

定義元: `backend/src/app/core/workspace_service.py:346`

```mermaid
sequenceDiagram
    participant Caller as 呼出元
    participant Function as unsave_recipe
    participant Callee as 呼出先
    Caller->>Function: self, request: RevisionRequest, row_id: UUID
    Function->>Callee: self._save_recipe(#39;unsave_recipe#39;, request, row_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: self._save_recipe(#39;unsave_recipe#39;, request, row_id)
    end
```
