...
sequenceDiagram
    ...
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Function->>Callee: q.run(#39;q001_recipe#39;, recipe_id=row_id, preview=local_auth_enabled())
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = q.run(#39;q001_recipe#39;, recipe_id=row_id, preview=local_auth_enabled())
    alt not rows
