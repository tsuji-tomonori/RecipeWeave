...
定義元: `backend/src/app/apis/workspace/create_custom_food/router.py:22`
...
sequenceDiagram
    ...
    Caller->>Function: identity: IdentityDependency, database: DatabaseDependency, request: CustomFoodRequest
...
定義元: `backend/src/app/apis/workspace/create_custom_food/functions.py:6`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: service: WorkspaceService, request: CustomFoodRequest
    Function->>Callee: service.create_custom_food(request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: service.create_custom_food(request)
    end
```
...
定義元: `backend/src/app/core/workspace_service.py:420`

```mermaid
sequenceDiagram
    participant Caller as 呼出元
    participant Function as create_custom_food
    participant Callee as 呼出先
    Caller->>Function: self, request: CustomFoodRequest
    Function->>Callee: self.begin(#39;create_custom_food#39;, request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: q = self.begin(#39;create_custom_food#39;, request)
    Function->>Callee: self._custom_food(q, request.food)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
