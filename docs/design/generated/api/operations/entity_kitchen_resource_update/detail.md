# 詳細設計: entity_kitchen_resource_update

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PUT /api/entities/kitchen_resource/{row_id}` — キッチンの実資源の更新
...
### `backend/src/app/apis/entities/kitchen_resource_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.kitchen_resource | U | id: 不変の行識別子; created_at: 作成日時（UTC）; user_id: 所有者; resource_type_id: コンロ・鍋・人等; name: 左コンロ・26cmフライパン等; capacity: 容量; quantity: 同等資源数; active: 新規の調理計画で利用する資源か |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND t.user_id = %(actor_id)s`

| SQLバインド | 実装上の値の出所 |
|---|---|
| active | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| capacity | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| resource_type_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/kitchen_resource_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
...
代入・選択式: `user_id = %(user_id)s; resource_type_id = %(resource_type_id)s; name = %(name)s; capacity = %(capacity)s; quantity = %(quantity)s; active = %(active)s`

### `backend/src/app/apis/entities/kitchen_resource_update/sql/002_reference_user_id.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/kitchen_resource_update/router.py:35 |
| execute | KitchenResourceRow.model_validate(rows[0]) | backend/src/app/apis/entities/kitchen_resource_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | キッチンの実資源の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/kitchen_resource_update/router.py:35 |
| execute | キッチンの実資源の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/kitchen_resource_update/functions.py:9 |
