# 詳細設計: entity_shopping_item_create
...
### `backend/src/app/apis/entities/shopping_item_create/sql/001_create.sql`
...
### `backend/src/app/apis/entities/shopping_item_create/sql/002_reference_session_id.sql`
...
### `backend/src/app/apis/entities/shopping_item_create/sql/003_reference_total_id.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/shopping_item_create/router.py:28 |
| execute | ShoppingItemRow.model_validate(rows[0]) | backend/src/app/apis/entities/shopping_item_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 買い物行の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/shopping_item_create/router.py:28 |
| execute | 買い物行の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/shopping_item_create/functions.py:7 |
