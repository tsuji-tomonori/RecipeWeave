# 詳細設計: entity_recipe_step_update

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PUT /api/entities/recipe_step/{row_id}` — 調理工程節点の更新
...
### `backend/src/app/apis/entities/recipe_step_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_step | U | id: 不変の行識別子; created_at: 作成日時（UTC）; recipe_version_id: 所属版; step_no: 表示順（依存順とは別）; operation_id: 標準動作; instruction: 個別補足; attention: 作業者拘束; duration_min_s: 所要秒下限; duration_max_s: 所要秒上限; scaling_rule_id: 時間の人数変更規則; completion_cue: 実測・目視の終了条件; title: 工程の短い見出し |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| attention | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| completion_cue | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| duration_max_s | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| duration_min_s | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| expected_etag | parse_etag(if_match) (backend/src/app/core/entity_service.py:72) |
| instruction | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| operation_id | CONTRACT.operation_id (backend/src/app/apis/entities/recipe_step_update/router.py:17) / spec.operation_id (backend/src/app/core/entity_service.py:50) / spec.operation_id (backend/src/app/core/entity_service.py:109) / spec.operation_id (backend/src/app/core/entity_service.py:119) |
| recipe_version_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/recipe_step_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
| scaling_rule_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| step_no | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| title | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `recipe_version_id = %(recipe_version_id)s; step_no = %(step_no)s; operation_id = %(operation_id)s; instruction = %(instruction)s; attention = %(attention)s; duration_min_s = %(duration_min_s)s; duration_max_s = %(duration_max_s)s; scaling_rule_id = %(scaling_rule_id)s; completion_cue = %(completion_cue)s; title = %(title)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_step_update/router.py:32 |
| execute | RecipeStepRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_step_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 調理工程節点の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_step_update/router.py:32 |
| execute | 調理工程節点の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_step_update/functions.py:9 |
