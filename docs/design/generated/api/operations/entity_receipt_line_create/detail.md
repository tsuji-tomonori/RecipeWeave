# 詳細設計: entity_receipt_line_create
...
### `backend/src/app/apis/entities/receipt_line_create/sql/001_create.sql`
...
### `backend/src/app/apis/entities/receipt_line_create/sql/002_reference_import_id.sql`
...
### `backend/src/app/apis/entities/receipt_line_create/sql/003_reference_pantry_lot_id.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/receipt_line_create/router.py:28 |
| execute | ReceiptLineRow.model_validate(rows[0]) | backend/src/app/apis/entities/receipt_line_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | レシートの商品候補と確定した在庫の対応の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/receipt_line_create/router.py:28 |
| execute | レシートの商品候補と確定した在庫の対応の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/receipt_line_create/functions.py:7 |
