# シーケンス: entity_generation_stratum_metric_list
...
定義元: `backend/src/app/apis/entities/generation_stratum_metric_list/router.py:31`
...
sequenceDiagram
    ...
    Caller->>Function: identity: IdentityDependency, database: DatabaseDependency, limit: Annotated[int, Query(ge=1, le=100)]=50, after: UUID | None=None
    Function->>Callee: EntityService(database, identity)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Function->>Callee: execute(EntityService(database, identity), limit, after)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: result = execute(EntityService(database, identity), limit, after)
...
定義元: `backend/src/app/apis/entities/generation_stratum_metric_list/functions.py:9`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: service: EntityService, limit: int=50, after: UUID | None=None
    Function->>Callee: service.execute(SPECIFICATIONS[#39;entity_generation_stratum_metric_list#39;], limit=limit, after=after)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = service.execute(SPECIFICATIONS[#39;entity_generation_stratum_metric_list#39;], limit=limit, after=after)
    Note over Function: 条件付き式を評価: [GenerationStratumMetricRow.model_validate(row) for row in rows]
    break この経路の関数終了: return
        Function-->>Caller: [GenerationStratumMetricRow.model_validate(row) for row in rows]
    end
```
