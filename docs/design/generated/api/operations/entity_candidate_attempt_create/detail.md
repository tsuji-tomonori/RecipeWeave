# 詳細設計: entity_candidate_attempt_create
...
### `backend/src/app/apis/entities/candidate_attempt_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.candidate_attempt | C | id: 不変ID; created_at: 作成日時; template_id: 定義版; ordinal: 設計点の序数; design_key: 正規化した設計キー; job_id: 生成ジョブ; state: 候補の段階; reason_code: 棄却理由; recipe_version_id: 採用した版; attempts: 試行上限（暫定） |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/candidate_attempt_create/router.py:31 |
| execute | CandidateAttemptRow.model_validate(rows[0]) | backend/src/app/apis/entities/candidate_attempt_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 試行済み設計点の台帳の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/candidate_attempt_create/router.py:31 |
| execute | 試行済み設計点の台帳の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/candidate_attempt_create/functions.py:7 |
