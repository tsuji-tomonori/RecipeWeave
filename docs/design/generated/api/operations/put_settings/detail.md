...
`PUT /api/settings` — 好み・常備食材・器具を設定する
...
### `backend/src/app/apis/workspace/put_settings/sql/q001_clear_exclusion.sql`
...
| SQLバインド | 実装上の値の出所 |
|---|---|
| user_id | self.user_id (backend/src/app/core/workspace_service.py:64) / self.user_id (backend/src/app/core/workspace_service.py:70) / self.user_id (backend/src/app/core/workspace_service.py:71) / self.user_id (backend/src/app/core/workspace_service.py:83) / self.user_id (backend/src/app/core/workspace_service.py:84) / self.user_id (backend/src/app/core/workspace_service.py:132) / self.user_id (backend/src/app/core/workspace_service.py:105) / self.user_id (backend/src/app/core/workspace_service.py:119) / self.user_id (backend/src/app/core/workspace_service.py:147) / self.user_id (backend/src/app/core/workspace_service.py:158) / self.user_id (backend/src/app/core/workspace_service.py:171) / self.user_id (backend/src/app/core/workspace_service.py:187) / self.user_id (backend/src/app/core/workspace_service.py:205) / self.user_id (backend/src/app/core/workspace_service.py:349) / self.user_id (backend/src/app/core/workspace_service.py:351) / self.user_id (backend/src/app/core/workspace_service.py:353) / self.user_id (backend/src/app/core/workspace_service.py:355) / self.user_id (backend/src/app/core/workspace_service.py:356) |

上記の条件に一致する行を削除する。参照先への削除動作はテーブル仕様の外部キー定義に従う。

### `backend/src/app/apis/workspace/put_settings/sql/q002_clear_pantry.sql`
...
### `backend/src/app/apis/workspace/put_settings/sql/q003_clear_equipment.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.kitchen_resource | U | user_id: 所有者; resource_type_id: コンロ・鍋・人等; active: 新規の調理計画で利用する資源か |
| recipeweave.resource_type | R | id: 不変の行識別子; code: burner/pan/person等 |

対象条件: `WHERE k.user_id = %(user_id)s AND EXISTS(SELECT 1 FROM recipeweave.resource_type AS r WHERE r.id = k.resource_type_id AND NOT r.code IN ('person', 'burner', 'bowl'))`
...
### `backend/src/app/apis/workspace/put_settings/sql/q004_exclusion.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.user_exclusion | C | id: 不変の行識別子; user_id: 利用者; food_id: 食材; allergen_id: アレルゲン; strict: 不明も除外するか |

対象条件: `SQL上の絞り込みなし`

| SQLバインド | 実装上の値の出所 |
|---|---|
| food_id | identifier(food) (backend/src/app/core/workspace_service.py:351) / identifier(food) (backend/src/app/core/workspace_service.py:353) |
| row_id | uuid4() (backend/src/app/core/workspace_service.py:71) / uuid4() (backend/src/app/core/workspace_service.py:351) / uuid4() (backend/src/app/core/workspace_service.py:353) / uuid4() (backend/src/app/core/workspace_service.py:356) |
| user_id | self.user_id (backend/src/app/core/workspace_service.py:64) / self.user_id (backend/src/app/core/workspace_service.py:70) / self.user_id (backend/src/app/core/workspace_service.py:71) / self.user_id (backend/src/app/core/workspace_service.py:83) / self.user_id (backend/src/app/core/workspace_service.py:84) / self.user_id (backend/src/app/core/workspace_service.py:132) / self.user_id (backend/src/app/core/workspace_service.py:105) / self.user_id (backend/src/app/core/workspace_service.py:119) / self.user_id (backend/src/app/core/workspace_service.py:147) / self.user_id (backend/src/app/core/workspace_service.py:158) / self.user_id (backend/src/app/core/workspace_service.py:171) / self.user_id (backend/src/app/core/workspace_service.py:187) / self.user_id (backend/src/app/core/workspace_service.py:205) / self.user_id (backend/src/app/core/workspace_service.py:349) / self.user_id (backend/src/app/core/workspace_service.py:351) / self.user_id (backend/src/app/core/workspace_service.py:353) / self.user_id (backend/src/app/core/workspace_service.py:355) / self.user_id (backend/src/app/core/workspace_service.py:356) |
...
### `backend/src/app/apis/workspace/put_settings/sql/q005_pantry.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.user_pantry_food | C | id: 不変の行識別子; user_id: 所有者; food_id: 常備食材 |
...
### `backend/src/app/apis/workspace/put_settings/sql/q006_equipment.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.kitchen_resource | U | id: 不変の行識別子; user_id: 所有者; resource_type_id: コンロ・鍋・人等; active: 新規の調理計画で利用する資源か |
| recipeweave.resource_type | R | id: 不変の行識別子; name: 道具名; status: 使用状態 |

対象条件: `WHERE kitchen.user_id = %(user_id)s AND kitchen.resource_type_id = resource_kind.id AND resource_kind.name = %(name)s AND resource_kind.status = 'active'`

| SQLバインド | 実装上の値の出所 |
|---|---|
| name | r['name'] (backend/src/app/core/workspace_service.py:137) / equipment (backend/src/app/core/workspace_service.py:355) / equipment (backend/src/app/core/workspace_service.py:356) |
| user_id | self.user_id (backend/src/app/core/workspace_service.py:64) / self.user_id (backend/src/app/core/workspace_service.py:70) / self.user_id (backend/src/app/core/workspace_service.py:71) / self.user_id (backend/src/app/core/workspace_service.py:83) / self.user_id (backend/src/app/core/workspace_service.py:84) / self.user_id (backend/src/app/core/workspace_service.py:132) / self.user_id (backend/src/app/core/workspace_service.py:105) / self.user_id (backend/src/app/core/workspace_service.py:119) / self.user_id (backend/src/app/core/workspace_service.py:147) / self.user_id (backend/src/app/core/workspace_service.py:158) / self.user_id (backend/src/app/core/workspace_service.py:171) / self.user_id (backend/src/app/core/workspace_service.py:187) / self.user_id (backend/src/app/core/workspace_service.py:205) / self.user_id (backend/src/app/core/workspace_service.py:349) / self.user_id (backend/src/app/core/workspace_service.py:351) / self.user_id (backend/src/app/core/workspace_service.py:353) / self.user_id (backend/src/app/core/workspace_service.py:355) / self.user_id (backend/src/app/core/workspace_service.py:356) |
...
### `backend/src/app/apis/workspace/put_settings/sql/q007_add_equipment.sql`
...
### `backend/src/app/apis/workspace/put_settings/sql/q900_lock_revision.sql`
...
### `backend/src/app/apis/workspace/put_settings/sql/q901_advance_revision.sql`
...
### `backend/src/app/apis/workspace/put_settings/sql/q902_append_audit.sql`
...
| not existing and (not q.run('q007_add_equipment', row_id=uuid4(), user_id=self.user_id, name=equipment)) | HTTPException(422, '未登録の器具が含まれています') | backend/src/app/core/workspace_service.py:345 |

## 出力

| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | execute(WorkspaceService(database, identity), request) | backend/src/app/apis/workspace/put_settings/router.py:22 |
| execute | service.put_settings(request) | backend/src/app/apis/workspace/put_settings/functions.py:6 |
...
| WorkspaceService.put_settings | self.finish(q) | backend/src/app/core/workspace_service.py:345 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 好み・常備食材・器具を設定する。呼出元が送った利用者IDは使用しない。 | backend/src/app/apis/workspace/put_settings/router.py:22 |
| execute | 好み・常備食材・器具を設定する。永続値は業務サービスが検証し、同一トランザクションで扱う。 | backend/src/app/apis/workspace/put_settings/functions.py:6 |
...
| WorkspaceService.put_settings | 除外・常備・器具の選択を関連行として保存する。 | backend/src/app/core/workspace_service.py:345 |
