# 詳細設計: entity_product_version_create
...
### `backend/src/app/apis/entities/product_version_create/sql/001_create.sql`
...
| SQLバインド | 実装上の値の出所 |
|---|---|
| drain_amount | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| form_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| net_amount | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| preparation_note | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| product_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| valid_from | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/product_version_create/router.py:28 |
| execute | ProductVersionRow.model_validate(rows[0]) | backend/src/app/apis/entities/product_version_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 商品仕様版の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/product_version_create/router.py:28 |
| execute | 商品仕様版の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/product_version_create/functions.py:7 |
