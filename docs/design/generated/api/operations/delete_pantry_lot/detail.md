...
### `backend/src/app/apis/workspace/delete_pantry_lot/sql/q001_delete_lot.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.pantry_lot | U | id: 不変の行識別子; user_id: 所有者; status: 在庫の有効・削除・レシート取消状態; updated_at: 最終編集日時 |

対象条件: `WHERE id = %(row_id)s AND user_id = %(user_id)s AND status = 'active'`

| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | uuid4() (backend/src/app/core/workspace_service.py:71) / row_id (backend/src/app/core/workspace_service.py:258) |
| user_id | self.user_id (backend/src/app/core/workspace_service.py:64) / self.user_id (backend/src/app/core/workspace_service.py:70) / self.user_id (backend/src/app/core/workspace_service.py:71) / self.user_id (backend/src/app/core/workspace_service.py:83) / self.user_id (backend/src/app/core/workspace_service.py:84) / self.user_id (backend/src/app/core/workspace_service.py:132) / self.user_id (backend/src/app/core/workspace_service.py:105) / self.user_id (backend/src/app/core/workspace_service.py:119) / self.user_id (backend/src/app/core/workspace_service.py:147) / self.user_id (backend/src/app/core/workspace_service.py:158) / self.user_id (backend/src/app/core/workspace_service.py:171) / self.user_id (backend/src/app/core/workspace_service.py:187) / self.user_id (backend/src/app/core/workspace_service.py:205) / self.user_id (backend/src/app/core/workspace_service.py:258) |
...
代入・選択式: `status = 'deleted'; updated_at = CURRENT_TIMESTAMP`

### `backend/src/app/apis/workspace/delete_pantry_lot/sql/q900_lock_revision.sql`
...
### `backend/src/app/apis/workspace/delete_pantry_lot/sql/q901_advance_revision.sql`
...
### `backend/src/app/apis/workspace/delete_pantry_lot/sql/q902_append_audit.sql`
...
| not q.run('q001_delete_lot', row_id=row_id, user_id=self.user_id) | HTTPException(404, '在庫が見つからないか削除済みです') | backend/src/app/core/workspace_service.py:255 |

## 出力

| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | execute(WorkspaceService(database, identity), request, row_id) | backend/src/app/apis/workspace/delete_pantry_lot/router.py:24 |
| execute | service.delete_pantry_lot(request, row_id) | backend/src/app/apis/workspace/delete_pantry_lot/functions.py:8 |
...
| WorkspaceService.delete_pantry_lot | self.finish(q) | backend/src/app/core/workspace_service.py:255 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 手持ち食材を削除する。呼出元が送った利用者IDは使用しない。 | backend/src/app/apis/workspace/delete_pantry_lot/router.py:24 |
| execute | 手持ち食材を削除する。永続値は業務サービスが検証し、同一トランザクションで扱う。 | backend/src/app/apis/workspace/delete_pantry_lot/functions.py:8 |
...
| WorkspaceService.delete_pantry_lot | 消費履歴を残し、在庫を利用対象から外す。 | backend/src/app/core/workspace_service.py:255 |
