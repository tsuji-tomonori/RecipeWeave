...
定義元: `backend/src/app/apis/workspace/delete_pantry_lot/router.py:24`
...
定義元: `backend/src/app/apis/workspace/delete_pantry_lot/functions.py:8`
...
sequenceDiagram
    ...
    Function->>Callee: service.delete_pantry_lot(request, row_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: service.delete_pantry_lot(request, row_id)
    end
```
...
### workspace_service.py: `delete_pantry_lot`

定義元: `backend/src/app/core/workspace_service.py:255`

```mermaid
sequenceDiagram
    participant Caller as 呼出元
    participant Function as delete_pantry_lot
    participant Callee as 呼出先
    Caller->>Function: self, request: RevisionRequest, row_id: UUID
    Function->>Callee: self.begin(#39;delete_pantry_lot#39;, request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: q = self.begin(#39;delete_pantry_lot#39;, request)
    Function->>Callee: q.run(#39;q001_delete_lot#39;, row_id=row_id, user_id=self.user_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    alt not q.run(#39;q001_delete_lot#39;, row_id=row_id, user_id=self.user_id)
        Function->>Callee: HTTPException(404, #39;在庫が見つからないか削除済みです#39;)
        Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
        break この経路の関数終了: raise
            Function-->>Caller: HTTPException(404, #39;在庫が見つからないか削除済みです#39;)
        end
    end
