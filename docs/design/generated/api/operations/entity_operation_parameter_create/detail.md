# 詳細設計: entity_operation_parameter_create
...
### `backend/src/app/apis/entities/operation_parameter_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.operation_parameter | C | id: 不変の行識別子; created_at: 作成日時（UTC）; operation_id: 動作; code: thickness_mm等; name: 厚さ等; value_type: 値型; unit_id: 単位; required: 必須か; min_value: 許容下限; max_value: 許容上限; allowed_values: option型の具体値配列 |
...
| operation_id | CONTRACT.operation_id (backend/src/app/apis/entities/operation_parameter_create/router.py:17) / spec.operation_id (backend/src/app/core/entity_service.py:50) / spec.operation_id (backend/src/app/core/entity_service.py:109) / spec.operation_id (backend/src/app/core/entity_service.py:119) |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/operation_parameter_create/router.py:31 |
| execute | OperationParameterRow.model_validate(rows[0]) | backend/src/app/apis/entities/operation_parameter_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 動作パラメータ定義の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/operation_parameter_create/router.py:31 |
| execute | 動作パラメータ定義の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/operation_parameter_create/functions.py:7 |
