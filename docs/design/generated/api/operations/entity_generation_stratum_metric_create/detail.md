# 詳細設計: entity_generation_stratum_metric_create
...
### `backend/src/app/apis/entities/generation_stratum_metric_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.generation_stratum_metric | C | id: 不変ID; created_at: 作成日時; template_id: 対象テンプレート; window_start: 計測窓開始; window_end: 計測窓終了; attempted: 試行数; valid: 適合生成数; unique_count: 既存集合との差分数; publishable: 公開基準通過数; input_tokens: 入力トークン合計; output_tokens: 出力トークン合計; cost_amount: 同一通貨の費用; currency: JPY/USD等; stratum_key: 層の安定キー（料理構造×食品カテゴリ×入手性）。集計定義はテンプレート版に固定 |

対象条件: `SQL上の絞り込みなし`

| SQLバインド | 実装上の値の出所 |
|---|---|
| attempted | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| cost_amount | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| currency | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| input_tokens | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| output_tokens | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| publishable | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| stratum_key | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| template_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| unique_count | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| valid | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| window_end | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| window_start | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/generation_stratum_metric_create/router.py:31 |
| execute | GenerationStratumMetricRow.model_validate(rows[0]) | backend/src/app/apis/entities/generation_stratum_metric_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 採用率・飽和度の実測の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/generation_stratum_metric_create/router.py:31 |
| execute | 採用率・飽和度の実測の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/generation_stratum_metric_create/functions.py:7 |
