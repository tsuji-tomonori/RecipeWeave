# 詳細設計: entity_step_media_update

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PUT /api/entities/step_media/{row_id}` — 工程別メディア選択の更新
...
### `backend/src/app/apis/entities/step_media_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.step_media | U | id: 不変の行識別子; created_at: 作成日時（UTC）; step_id: 対象工程; media_id: 適用メディア; start_ms: 表示開始点; end_ms: 終了点 |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| end_ms | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| expected_etag | parse_etag(if_match) (backend/src/app/core/entity_service.py:72) |
| media_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/step_media_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
| start_ms | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `step_id = %(step_id)s; media_id = %(media_id)s; start_ms = %(start_ms)s; end_ms = %(end_ms)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/step_media_update/router.py:32 |
| execute | StepMediaRow.model_validate(rows[0]) | backend/src/app/apis/entities/step_media_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 工程別メディア選択の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/step_media_update/router.py:32 |
| execute | 工程別メディア選択の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/step_media_update/functions.py:9 |
