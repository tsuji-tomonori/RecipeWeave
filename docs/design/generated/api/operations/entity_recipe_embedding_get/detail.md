# 詳細設計: entity_recipe_embedding_get

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`GET /api/entities/recipe_embedding/{row_id}` — 近似検索用特徴量の取得
...
### `backend/src/app/apis/entities/recipe_embedding_get/sql/001_get.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_embedding | R | id: 不変ID; created_at: 作成日時; recipe_version_id: 対象版; model_version: 埋め込みモデル固定版; content_hash: 入力内容ハッシュ; embedding: 仮定768次元float32; created_for_index: 検索索引版 |

対象条件: `WHERE t.id = %(row_id)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | row_id (backend/src/app/apis/entities/recipe_embedding_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |

代入・選択式: `t.id; t.created_at; t.recipe_version_id; t.model_version; t.content_hash; t.embedding; t.created_for_index; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_embedding_get/router.py:31 |
| execute | RecipeEmbeddingRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_embedding_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 近似検索用特徴量の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_embedding_get/router.py:31 |
| execute | 近似検索用特徴量の取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_embedding_get/functions.py:9 |
