# シーケンス: entity_receipt_import_delete
...
定義元: `backend/src/app/apis/entities/receipt_import_delete/router.py:32`
...
定義元: `backend/src/app/apis/entities/receipt_import_delete/functions.py:9`
...
sequenceDiagram
    ...
    Function->>Callee: service.execute(SPECIFICATIONS[#39;entity_receipt_import_delete#39;], row_id=row_id, if_match=if_match)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = service.execute(SPECIFICATIONS[#39;entity_receipt_import_delete#39;], row_id=row_id, if_match=if_match)
