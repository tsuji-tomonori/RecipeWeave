# 詳細設計: entity_receipt_import_delete

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`DELETE /api/entities/receipt_import/{row_id}` — レシート読取・在庫登録の処理単位の削除
...
### `backend/src/app/apis/entities/receipt_import_delete/sql/001_delete.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.receipt_import | D | id: 不変の行識別子; created_at: 作成日時（UTC）; user_id: 所有者; file_sha256: 画像本文のSHA256。本文はDBに保存しない; idempotency_key: 本人内で一意の再送防止キー; status: draft/committed/revertedの状態; revision: 楽観ロック版; committed_at: 在庫へ登録した日時; reverted_at: 登録取消日時; undo_preserved_count: レシート取消時に編集・消費済みとして残した在庫件数 |
...
| row_id | row_id (backend/src/app/apis/entities/receipt_import_delete/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/receipt_import_delete/router.py:32 |
| execute | ReceiptImportRow.model_validate(rows[0]) | backend/src/app/apis/entities/receipt_import_delete/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | レシート読取・在庫登録の処理単位の削除。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/receipt_import_delete/router.py:32 |
| execute | レシート読取・在庫登録の処理単位の削除を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/receipt_import_delete/functions.py:9 |
