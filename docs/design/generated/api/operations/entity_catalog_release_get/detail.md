# 詳細設計: entity_catalog_release_get
...
### `backend/src/app/apis/entities/catalog_release_get/sql/001_get.sql`
...
| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | row_id (backend/src/app/apis/entities/catalog_release_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |

代入・選択式: `t.id; t.created_at; t.version; t.manifest_hash; t.published_at; t.owner_id; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/catalog_release_get/router.py:31 |
| execute | CatalogReleaseRow.model_validate(rows[0]) | backend/src/app/apis/entities/catalog_release_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | カタログ公開版の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/catalog_release_get/router.py:31 |
| execute | カタログ公開版の取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/catalog_release_get/functions.py:9 |
