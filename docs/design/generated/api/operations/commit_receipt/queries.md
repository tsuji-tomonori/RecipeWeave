...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.food | R | id |
| recipeweave.food_form | R | food_id, id, name, status |
| recipeweave.unit | R | code, id, status |
| recipeweave.user_food | R | food_id, user_id |

バインド変数: food_id, form, unit, user_id
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.pantry_lot | C | amount, expires_on, form_id, id, location, original_amount, original_form_id, original_unit_id, priority, quantity_quality, source_import_id, unit_id, user_id |

バインド変数: amount, expires_on, form_id, import_id, location, priority, quality, row_id, unit_id, user_id
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.receipt_import | R | created_at, file_sha256, id, idempotency_key, status, user_id |

バインド変数: hash, import_id, signature, user_id
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.receipt_import | C | committed_at, file_sha256, id, idempotency_key, status, user_id |

バインド変数: hash, import_id, key, user_id
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.receipt_line | C | amount, decision, form_id, id, import_id, line_no, pantry_lot_id, raw_name, unit_id |

バインド変数: amount, form_id, import_id, line_no, lot_id, name, row_id, unit_id
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.catalog_release | C | id, manifest_hash, owner_id, published_at, version |

バインド変数: manifest, release_id, user_id, version
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.food | C | code, id, kind, name, owner_id, parent_id, release_id, status |

バインド変数: code, food_id, name, release_id, user_id
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.user_food | C | food_id, id, user_id |

バインド変数: food_id, row_id, user_id
...
| 対象表 | CRUD | 参照・書込列 |
|---|---|---|
| recipeweave.food_form | C | base_unit_id, food_id, id, name, quantity_basis, state, status |
| recipeweave.unit | R | code, id, status |

バインド変数: food_id, row_id, unit
