# 詳細設計: entity_recipe_embedding_create

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`POST /api/entities/recipe_embedding` — 近似検索用特徴量の作成
...
| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_embedding | C | id: 不変ID; created_at: 作成日時; recipe_version_id: 対象版; model_version: 埋め込みモデル固定版; content_hash: 入力内容ハッシュ; embedding: 仮定768次元float32; created_for_index: 検索索引版 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_embedding_create/router.py:31 |
| execute | RecipeEmbeddingRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_embedding_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 近似検索用特徴量の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_embedding_create/router.py:31 |
| execute | 近似検索用特徴量の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_embedding_create/functions.py:7 |
