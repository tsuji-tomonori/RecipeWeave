# 詳細設計: entity_generation_policy_create
...
### `backend/src/app/apis/entities/generation_policy_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.generation_policy | C | id: 不変の行識別子; created_at: 作成日時（UTC）; version: 方針識別子; prompt_template: 入力テンプレ; model_identifier: 利用モデル名・版; parameter_json: temperature/seed等の記録; schema_version: 出力JSON契約; release_id: 候補カタログ版 |

対象条件: `SQL上の絞り込みなし`

| SQLバインド | 実装上の値の出所 |
|---|---|
| model_identifier | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| parameter_json | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| prompt_template | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| schema_version | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/generation_policy_create/router.py:31 |
| execute | GenerationPolicyRow.model_validate(rows[0]) | backend/src/app/apis/entities/generation_policy_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | AI生成方針版の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/generation_policy_create/router.py:31 |
| execute | AI生成方針版の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/generation_policy_create/functions.py:7 |
