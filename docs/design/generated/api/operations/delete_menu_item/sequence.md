...
定義元: `backend/src/app/apis/workspace/delete_menu_item/router.py:24`
...
定義元: `backend/src/app/apis/workspace/delete_menu_item/functions.py:8`
...
sequenceDiagram
    ...
    Function->>Callee: service.delete_menu_item(request, row_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: service.delete_menu_item(request, row_id)
    end
```
...
定義元: `backend/src/app/core/workspace_service.py:321`

```mermaid
sequenceDiagram
    participant Caller as 呼出元
    participant Function as delete_menu_item
    participant Callee as 呼出先
    Caller->>Function: self, request: RevisionRequest, row_id: UUID
    Function->>Callee: self.begin(#39;delete_menu_item#39;, request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: q = self.begin(#39;delete_menu_item#39;, request)
