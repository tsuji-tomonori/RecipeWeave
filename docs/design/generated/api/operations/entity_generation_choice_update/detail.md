# 詳細設計: entity_generation_choice_update

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PUT /api/entities/generation_choice/{row_id}` — 生成軸の選択値の更新
...
### `backend/src/app/apis/entities/generation_choice_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.generation_choice | U | id: 不変の行識別子; created_at: 作成日時（UTC）; job_id: 実行; option_id: 選択した軸候補 |
...
| row_id | row_id (backend/src/app/apis/entities/generation_choice_update/functions.py:16) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
...
代入・選択式: `job_id = %(job_id)s; option_id = %(option_id)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/generation_choice_update/router.py:35 |
| execute | GenerationChoiceRow.model_validate(rows[0]) | backend/src/app/apis/entities/generation_choice_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 生成軸の選択値の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/generation_choice_update/router.py:35 |
| execute | 生成軸の選択値の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/generation_choice_update/functions.py:9 |
