...
### `backend/src/app/apis/entities/food_form_create/sql/001_create.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/food_form_create/router.py:28 |
| execute | FoodFormRow.model_validate(rows[0]) | backend/src/app/apis/entities/food_form_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 食材形態の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/food_form_create/router.py:28 |
| execute | 食材形態の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/food_form_create/functions.py:7 |
