...
定義元: `backend/src/app/apis/entities/nutrient_update/router.py:32`
...
sequenceDiagram
    ...
    Caller->>Function: response: Response, identity: IdentityDependency, database: DatabaseDependency, row_id: UUID, payload: NutrientWrite, if_match: Annotated[str | None, Header(alias=#39;If-Match#39;)]=None
...
定義元: `backend/src/app/apis/entities/nutrient_update/functions.py:9`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: service: EntityService, row_id: UUID, payload: NutrientWrite, if_match: str | None=None
    Function->>Callee: payload.model_dump(mode=#39;python#39;, by_alias=True)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Function->>Callee: service.execute(SPECIFICATIONS[#39;entity_nutrient_update#39;], row_id=row_id, payload=payload.model_dump(mode=#39;python#39;, by_alias=True), if_match=if_match)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = service.execute(SPECIFICATIONS[#39;entity_nutrient_update#39;], row_id=row_id, payload=payload.model_dump(mode=#39;python#39;, by_alias=True), if_match=if_match)
    Function->>Callee: NutrientRow.model_validate(rows[0])
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: NutrientRow.model_validate(rows[0])
    end
```
