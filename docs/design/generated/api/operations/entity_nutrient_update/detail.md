...
### `backend/src/app/apis/entities/nutrient_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.nutrient | U | id: 不変の行識別子; created_at: 作成日時（UTC）; code: energy_kcal等; name: エネルギー等; unit_label: kcal/g/mg/μg |
...
| row_id | row_id (backend/src/app/apis/entities/nutrient_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
| unit_label | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `code = %(code)s; name = %(name)s; unit_label = %(unit_label)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/nutrient_update/router.py:32 |
| execute | NutrientRow.model_validate(rows[0]) | backend/src/app/apis/entities/nutrient_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 栄養成分種別の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/nutrient_update/router.py:32 |
| execute | 栄養成分種別の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/nutrient_update/functions.py:9 |
