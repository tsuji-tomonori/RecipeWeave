# 詳細設計: entity_generation_job_create
...
### `backend/src/app/apis/entities/generation_job_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.generation_job | C | id: 不変の行識別子; created_at: 作成日時（UTC）; policy_id: 実行方針; idempotency_key: 入力と方針から作る重複キー; status: 進行状態; started_at: 開始; finished_at: 終了; seed: 再現用seed; error_code: 失敗分類; attempt_count: 試行回数 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/generation_job_create/router.py:28 |
| execute | GenerationJobRow.model_validate(rows[0]) | backend/src/app/apis/entities/generation_job_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 事前生成ジョブの作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/generation_job_create/router.py:28 |
| execute | 事前生成ジョブの作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/generation_job_create/functions.py:7 |
