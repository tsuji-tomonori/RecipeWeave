# 詳細設計: entity_compatibility_rule_create

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`POST /api/entities/compatibility_rule` — 組み合わせ・公開ルールの作成
...
### `backend/src/app/apis/entities/compatibility_rule_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.compatibility_rule | C | id: 不変の行識別子; created_at: 作成日時（UTC）; code: 規則コード; version: 規則版; severity: 除外/保留/順位; predicate: 型付き条件式; message: 理由; source_id: 根拠; status: 利用状態 |
...
| message | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| predicate | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| severity | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/compatibility_rule_create/router.py:31 |
| execute | CompatibilityRuleRow.model_validate(rows[0]) | backend/src/app/apis/entities/compatibility_rule_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 組み合わせ・公開ルールの作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/compatibility_rule_create/router.py:31 |
| execute | 組み合わせ・公開ルールの作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/compatibility_rule_create/functions.py:7 |
