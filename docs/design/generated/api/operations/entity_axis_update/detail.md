# 詳細設計: entity_axis_update

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PUT /api/entities/axis/{row_id}` — 組み合わせ軸の更新
...
### `backend/src/app/apis/entities/axis_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.axis | U | id: 不変の行識別子; created_at: 作成日時（UTC）; code: 軸コード; name: 軸名; purpose: 生成/検索/制約等; selection: 単複; release_id: 定義版; status: 採用状態 |
...
| purpose | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| release_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/axis_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
| selection | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `code = %(code)s; name = %(name)s; purpose = %(purpose)s; selection = %(selection)s; release_id = %(release_id)s; status = %(status)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/axis_update/router.py:32 |
| execute | AxisRow.model_validate(rows[0]) | backend/src/app/apis/entities/axis_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 組み合わせ軸の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/axis_update/router.py:32 |
| execute | 組み合わせ軸の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/axis_update/functions.py:9 |
