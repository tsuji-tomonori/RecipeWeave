# 詳細設計: entity_user_exclusion_update

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PUT /api/entities/user_exclusion/{row_id}` — 避けたい食材・物質の更新
...
### `backend/src/app/apis/entities/user_exclusion_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.user_exclusion | U | id: 不変の行識別子; created_at: 作成日時（UTC）; user_id: 利用者; food_id: 食材; allergen_id: アレルゲン; strict: 不明も除外するか |
...
| allergen_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| row_id | row_id (backend/src/app/apis/entities/user_exclusion_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
| strict | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `user_id = %(user_id)s; food_id = %(food_id)s; allergen_id = %(allergen_id)s; strict = %(strict)s`

### `backend/src/app/apis/entities/user_exclusion_update/sql/002_reference_user_id.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/user_exclusion_update/router.py:32 |
| execute | UserExclusionRow.model_validate(rows[0]) | backend/src/app/apis/entities/user_exclusion_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 避けたい食材・物質の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/user_exclusion_update/router.py:32 |
| execute | 避けたい食材・物質の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/user_exclusion_update/functions.py:9 |
