...
定義元: `backend/src/app/apis/entities/user_food_create/router.py:28`
...
sequenceDiagram
    ...
    Caller->>Function: response: Response, identity: IdentityDependency, database: DatabaseDependency, payload: UserFoodWrite
...
定義元: `backend/src/app/apis/entities/user_food_create/functions.py:7`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: service: EntityService, payload: UserFoodWrite
    Function->>Callee: payload.model_dump(mode=#39;python#39;, by_alias=True)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Function->>Callee: service.execute(SPECIFICATIONS[#39;entity_user_food_create#39;], payload=payload.model_dump(mode=#39;python#39;, by_alias=True))
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: rows = service.execute(SPECIFICATIONS[#39;entity_user_food_create#39;], payload=payload.model_dump(mode=#39;python#39;, by_alias=True))
