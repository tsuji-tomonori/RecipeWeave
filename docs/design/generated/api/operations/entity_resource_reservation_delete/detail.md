# 詳細設計: entity_resource_reservation_delete

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`DELETE /api/entities/resource_reservation/{row_id}` — 資源の予約の削除
...
### `backend/src/app/apis/entities/resource_reservation_delete/sql/001_delete.sql`
...
| recipeweave.resource_reservation | D | id: 不変の行識別子; created_at: 作成日時（UTC）; task_id: 使用タスク; resource_id: 実資源; start_s: 占有開始; end_s: 占有終了; quantity: 占有量 |
...
| row_id | row_id (backend/src/app/apis/entities/resource_reservation_delete/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/resource_reservation_delete/router.py:32 |
| execute | ResourceReservationRow.model_validate(rows[0]) | backend/src/app/apis/entities/resource_reservation_delete/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 資源の予約の削除。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/resource_reservation_delete/router.py:32 |
| execute | 資源の予約の削除を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/resource_reservation_delete/functions.py:9 |
