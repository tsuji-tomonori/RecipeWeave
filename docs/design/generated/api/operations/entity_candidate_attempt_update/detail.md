# 詳細設計: entity_candidate_attempt_update

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PUT /api/entities/candidate_attempt/{row_id}` — 試行済み設計点の台帳の更新
...
### `backend/src/app/apis/entities/candidate_attempt_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.candidate_attempt | U | id: 不変ID; created_at: 作成日時; template_id: 定義版; ordinal: 設計点の序数; design_key: 正規化した設計キー; job_id: 生成ジョブ; state: 候補の段階; reason_code: 棄却理由; recipe_version_id: 採用した版; attempts: 試行上限（暫定） |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| attempts | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| design_key | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| expected_etag | parse_etag(if_match) (backend/src/app/core/entity_service.py:71) |
| job_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| ordinal | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| reason_code | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| recipe_version_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/candidate_attempt_update/functions.py:16) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
| state | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| template_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `template_id = %(template_id)s; ordinal = %(ordinal)s; design_key = %(design_key)s; job_id = %(job_id)s; state = %(state)s; reason_code = %(reason_code)s; recipe_version_id = %(recipe_version_id)s; attempts = %(attempts)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/candidate_attempt_update/router.py:35 |
| execute | CandidateAttemptRow.model_validate(rows[0]) | backend/src/app/apis/entities/candidate_attempt_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 試行済み設計点の台帳の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/candidate_attempt_update/router.py:35 |
| execute | 試行済み設計点の台帳の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/candidate_attempt_update/functions.py:9 |
