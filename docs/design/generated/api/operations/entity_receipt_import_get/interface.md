# インターフェース: entity_receipt_import_get
...
"""レシート読取・在庫登録の処理単位の取得。具体例は専用型と操作別受入テストに対応する。"""
...
```json
{
  "description": "レシート読取・在庫登録の処理単位の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。",
  "operationId": "entity_receipt_import_get",
  "parameters": [
    {
      "in": "path",
      "name": "row_id",
