...
定義元: `backend/src/app/apis/workspace/get_workspace/router.py:22`
...
sequenceDiagram
    ...
    Function->>Callee: execute(WorkspaceService(database, identity))
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: execute(WorkspaceService(database, identity))
    end
```

### functions.py: `execute`

定義元: `backend/src/app/apis/workspace/get_workspace/functions.py:5`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: service: WorkspaceService
    Function->>Callee: service.get_workspace()
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: service.get_workspace()
    end
```
