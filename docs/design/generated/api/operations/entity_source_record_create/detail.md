# 詳細設計: entity_source_record_create
...
### `backend/src/app/apis/entities/source_record_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.source_record | C | id: 不変の行識別子; created_at: 作成日時（UTC）; title: 根拠名; url: 公式資料URL; locator: 資料内位置; retrieved_at: 取得時点; content_hash: 参照内容のハッシュ; license_note: 利用条件・権利確認 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/source_record_create/router.py:28 |
| execute | SourceRecordRow.model_validate(rows[0]) | backend/src/app/apis/entities/source_record_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 根拠資料の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/source_record_create/router.py:28 |
| execute | 根拠資料の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/source_record_create/functions.py:7 |
