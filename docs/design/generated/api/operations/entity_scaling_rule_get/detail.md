# 詳細設計: entity_scaling_rule_get

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`GET /api/entities/scaling_rule/{row_id}` — 人数変更規則の取得
...
### `backend/src/app/apis/entities/scaling_rule_get/sql/001_get.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.scaling_rule | R | id: 不変の行識別子; created_at: 作成日時（UTC）; name: 規則名; mode: 比例・バッチ等; min_servings: 検証済み人数下限; max_servings: 検証済み人数上限; batch_capacity: 1バッチ上限; round_mode: 表示丸め; round_increment: 表示・購入の刻み; source_id: 検証根拠 |

対象条件: `WHERE t.id = %(row_id)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | row_id (backend/src/app/apis/entities/scaling_rule_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |

代入・選択式: `t.id; t.created_at; t.name; t.mode; t.min_servings; t.max_servings; t.batch_capacity; t.round_mode; t.round_increment; t.source_id; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/scaling_rule_get/router.py:31 |
| execute | ScalingRuleRow.model_validate(rows[0]) | backend/src/app/apis/entities/scaling_rule_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 人数変更規則の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/scaling_rule_get/router.py:31 |
| execute | 人数変更規則の取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/scaling_rule_get/functions.py:9 |
