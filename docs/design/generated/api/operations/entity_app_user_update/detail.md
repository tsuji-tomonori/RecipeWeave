...
### `backend/src/app/apis/entities/app_user_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.app_user | U | id: 不変の行識別子; created_at: 作成日時（UTC）; auth_subject: 認証基盤の不透明識別子; state: 利用/削除処理; locale: 表示言語; timezone: IANAタイムゾーン |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND t.id = %(actor_id)s`
...
| row_id | row_id (backend/src/app/apis/entities/app_user_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
| timezone | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/app_user_update/router.py:32 |
| execute | AppUserRow.model_validate(rows[0]) | backend/src/app/apis/entities/app_user_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | アプリ利用者の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/app_user_update/router.py:32 |
| execute | アプリ利用者の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/app_user_update/functions.py:9 |
