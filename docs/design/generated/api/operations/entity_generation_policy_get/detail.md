# 詳細設計: entity_generation_policy_get

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`GET /api/entities/generation_policy/{row_id}` — AI生成方針版の取得
...
### `backend/src/app/apis/entities/generation_policy_get/sql/001_get.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.generation_policy | R | id: 不変の行識別子; created_at: 作成日時（UTC）; version: 方針識別子; prompt_template: 入力テンプレ; model_identifier: 利用モデル名・版; parameter_json: temperature/seed等の記録; schema_version: 出力JSON契約; release_id: 候補カタログ版 |

対象条件: `WHERE t.id = %(row_id)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | row_id (backend/src/app/apis/entities/generation_policy_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |

代入・選択式: `t.id; t.created_at; t.version; t.prompt_template; t.model_identifier; t.parameter_json; t.schema_version; t.release_id; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/generation_policy_get/router.py:31 |
| execute | GenerationPolicyRow.model_validate(rows[0]) | backend/src/app/apis/entities/generation_policy_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | AI生成方針版の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/generation_policy_get/router.py:31 |
| execute | AI生成方針版の取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/generation_policy_get/functions.py:9 |
