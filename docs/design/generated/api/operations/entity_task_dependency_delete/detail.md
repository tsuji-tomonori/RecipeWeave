# 詳細設計: entity_task_dependency_delete

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`DELETE /api/entities/task_dependency/{row_id}` — 献立展開後依存の削除
...
### `backend/src/app/apis/entities/task_dependency_delete/sql/001_delete.sql`
...
| recipeweave.task_dependency | D | id: 不変の行識別子; created_at: 作成日時（UTC）; before_task_id: 先行タスク; after_task_id: 後続タスク; min_lag_s: 最小間隔; max_lag_s: 最大間隔; reason: 元DAG/洗浄/設備切替等 |
...
| row_id | row_id (backend/src/app/apis/entities/task_dependency_delete/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/task_dependency_delete/router.py:32 |
| execute | TaskDependencyRow.model_validate(rows[0]) | backend/src/app/apis/entities/task_dependency_delete/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 献立展開後依存の削除。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/task_dependency_delete/router.py:32 |
| execute | 献立展開後依存の削除を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/task_dependency_delete/functions.py:9 |
