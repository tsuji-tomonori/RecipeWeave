# インターフェース: entity_task_dependency_delete
...
"""献立展開後依存の削除。具体例は専用型と操作別受入テストに対応する。"""
...
```json
{
  "description": "献立展開後依存の削除。認証情報は依存から取得し、本人所有または管理者権限を検査する。",
  "operationId": "entity_task_dependency_delete",
  "parameters": [
    {
      "in": "path",
      "name": "row_id",
