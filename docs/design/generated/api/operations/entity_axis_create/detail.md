...
### `backend/src/app/apis/entities/axis_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.axis | C | id: 不変の行識別子; created_at: 作成日時（UTC）; code: 軸コード; name: 軸名; purpose: 生成/検索/制約等; selection: 単複; release_id: 定義版; status: 採用状態 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/axis_create/router.py:28 |
| execute | AxisRow.model_validate(rows[0]) | backend/src/app/apis/entities/axis_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 組み合わせ軸の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/axis_create/router.py:28 |
| execute | 組み合わせ軸の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/axis_create/functions.py:7 |
