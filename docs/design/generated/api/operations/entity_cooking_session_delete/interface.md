# インターフェース: entity_cooking_session_delete
...
"""調理計画実行の削除。具体例は専用型と操作別受入テストに対応する。"""
...
```json
{
  "description": "調理計画実行の削除。認証情報は依存から取得し、本人所有または管理者権限を検査する。",
  "operationId": "entity_cooking_session_delete",
  "parameters": [
    {
      "in": "path",
      "name": "row_id",
