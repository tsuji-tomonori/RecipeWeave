# 詳細設計: entity_product_preparation_rule_create
...
### `backend/src/app/apis/entities/product_preparation_rule_create/sql/001_create.sql`
...
| SQLバインド | 実装上の値の出所 |
|---|---|
| allowed | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| operation_id | CONTRACT.operation_id (backend/src/app/apis/entities/product_preparation_rule_create/router.py:17) / spec.operation_id (backend/src/app/core/entity_service.py:50) / spec.operation_id (backend/src/app/core/entity_service.py:109) / spec.operation_id (backend/src/app/core/entity_service.py:119) |
...
| use_original_container | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/product_preparation_rule_create/router.py:31 |
| execute | ProductPreparationRuleRow.model_validate(rows[0]) | backend/src/app/apis/entities/product_preparation_rule_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 商品固有の調理条件の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/product_preparation_rule_create/router.py:31 |
| execute | 商品固有の調理条件の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/product_preparation_rule_create/functions.py:7 |
