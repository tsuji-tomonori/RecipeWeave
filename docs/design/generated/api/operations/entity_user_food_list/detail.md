...
### `backend/src/app/apis/entities/user_food_list/sql/001_list.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/user_food_list/router.py:31 |
| execute | [UserFoodRow.model_validate(row) for row in rows] | backend/src/app/apis/entities/user_food_list/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 利用者が追加した独自食材の所有の一覧。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/user_food_list/router.py:31 |
| execute | 利用者が追加した独自食材の所有の一覧を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/user_food_list/functions.py:9 |
