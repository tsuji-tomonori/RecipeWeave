# 詳細設計: entity_cooking_session_update
...
### `backend/src/app/apis/entities/cooking_session_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.cooking_session | U | id: 不変の行識別子; created_at: 作成日時（UTC）; menu_id: 対象献立; menu_revision: 献立版; status: 実行状態; target_at: 完成希望時刻; planner_version: 計画器の版; input_snapshot: 材料・資源・人数の固定入力; input_hash: 入力ハッシュ; current_task_index: 調理画面の現在の工程位置（0始まり） |
...
| current_task_index | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| expected_etag | parse_etag(if_match) (backend/src/app/core/entity_service.py:72) |
| input_hash | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| input_snapshot | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| menu_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| menu_revision | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| planner_version | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/cooking_session_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
| status | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| target_at | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `menu_id = %(menu_id)s; menu_revision = %(menu_revision)s; status = %(status)s; target_at = %(target_at)s; planner_version = %(planner_version)s; input_snapshot = %(input_snapshot)s; input_hash = %(input_hash)s; current_task_index = %(current_task_index)s`

### `backend/src/app/apis/entities/cooking_session_update/sql/002_reference_menu_id.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/cooking_session_update/router.py:32 |
| execute | CookingSessionRow.model_validate(rows[0]) | backend/src/app/apis/entities/cooking_session_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 調理計画実行の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/cooking_session_update/router.py:32 |
| execute | 調理計画実行の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/cooking_session_update/functions.py:9 |
