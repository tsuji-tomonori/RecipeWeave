...
定義元: `backend/src/app/apis/workspace/put_shopping_checks/router.py:22`
...
sequenceDiagram
    ...
    Caller->>Function: identity: IdentityDependency, database: DatabaseDependency, request: ShoppingRequest
...
定義元: `backend/src/app/apis/workspace/put_shopping_checks/functions.py:6`
...
sequenceDiagram
    ...
    participant Callee as 呼出先
    Caller->>Function: service: WorkspaceService, request: ShoppingRequest
    Function->>Callee: service.put_shopping_checks(request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    break この経路の関数終了: return
        Function-->>Caller: service.put_shopping_checks(request)
    end
```
...
定義元: `backend/src/app/core/workspace_service.py:367`

```mermaid
sequenceDiagram
    participant Caller as 呼出元
    participant Function as put_shopping_checks
    participant Callee as 呼出先
    Caller->>Function: self, request: ShoppingRequest
    Function->>Callee: self.begin(#39;put_shopping_checks#39;, request)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    Note over Function: q = self.begin(#39;put_shopping_checks#39;, request)
    Function->>Callee: q.run(#39;q001_clear#39;, user_id=self.user_id)
    Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
    loop item in request.checks
        rect rgb(244, 247, 246)
        Note over Function: try: 例外発生時は一致するexceptへ移る
            Function->>Callee: item.checked_at.replace(#39;Z#39;, #39;+00:00#39;)
            Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
            Function->>Callee: datetime.fromisoformat(item.checked_at.replace(#39;Z#39;, #39;+00:00#39;))
            Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
            Note over Function: checked_at = datetime.fromisoformat(item.checked_at.replace(#39;Z#39;, #39;+00:00#39;))
        end
        opt 例外: ValueError
            Function->>Callee: HTTPException(422, #39;購入確認日時が不正です#39;)
            Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
            break この経路の関数終了: raise
                Function-->>Caller: HTTPException(422, #39;購入確認日時が不正です#39;)
            end
        end
        Function->>Callee: uuid4()
        Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
        Function->>Callee: identifier(item.food_id)
        Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
        Function->>Callee: q.run(#39;q002_insert#39;, row_id=uuid4(), user_id=self.user_id, key=item.key, signature=item.signature, food_id=identifier(item.food_id), amount=item.quantity.value, unit=item.quantity.unit, checked_at=checked_at, archived=item.archived)
        Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
        alt not q.run(#39;q002_insert#39;, row_id=uuid4(), user_id=self.user_id, key=item.key, signature=item.signature, food_id=identifier(item.food_id), amount=item.quantity.value, unit=item.quantity.unit, checked_at=checked_at, archived=item.archived)
            Function->>Callee: HTTPException(422, #39;買い物の単位が見つかりません#39;)
            Callee-->>Function: 呼出結果（例外は呼出元へ伝播）
            break この経路の関数終了: raise
                Function-->>Caller: HTTPException(422, #39;買い物の単位が見つかりません#39;)
            end
        end
    end
