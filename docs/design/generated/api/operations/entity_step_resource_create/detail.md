# 詳細設計: entity_step_resource_create
...
### `backend/src/app/apis/entities/step_resource_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.step_resource | C | id: 不変の行識別子; created_at: 作成日時（UTC）; step_id: 対象工程; resource_type_id: 要求種別; quantity: 必要台数・人数; capacity_min: 必要最低容量; exclusive: 占有するか |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/step_resource_create/router.py:28 |
| execute | StepResourceRow.model_validate(rows[0]) | backend/src/app/apis/entities/step_resource_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 工程の資源要求の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/step_resource_create/router.py:28 |
| execute | 工程の資源要求の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/step_resource_create/functions.py:7 |
