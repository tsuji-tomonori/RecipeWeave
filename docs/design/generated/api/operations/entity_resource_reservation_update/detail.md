# 詳細設計: entity_resource_reservation_update

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PUT /api/entities/resource_reservation/{row_id}` — 資源の予約の更新
...
### `backend/src/app/apis/entities/resource_reservation_update/sql/001_update.sql`
...
| end_s | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| expected_etag | parse_etag(if_match) (backend/src/app/core/entity_service.py:72) |
| quantity | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| resource_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/resource_reservation_update/functions.py:16) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
| start_s | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| task_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `task_id = %(task_id)s; resource_id = %(resource_id)s; start_s = %(start_s)s; end_s = %(end_s)s; quantity = %(quantity)s`

### `backend/src/app/apis/entities/resource_reservation_update/sql/002_reference_task_id.sql`
...
### `backend/src/app/apis/entities/resource_reservation_update/sql/003_reference_resource_id.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.kitchen_resource | R | id: 不変の行識別子; user_id: 所有者 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/resource_reservation_update/router.py:35 |
| execute | ResourceReservationRow.model_validate(rows[0]) | backend/src/app/apis/entities/resource_reservation_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 資源の予約の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/resource_reservation_update/router.py:35 |
| execute | 資源の予約の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/resource_reservation_update/functions.py:9 |
