# 詳細設計: entity_receipt_import_update
...
### `backend/src/app/apis/entities/receipt_import_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.receipt_import | U | id: 不変の行識別子; created_at: 作成日時（UTC）; user_id: 所有者; file_sha256: 画像本文のSHA256。本文はDBに保存しない; idempotency_key: 本人内で一意の再送防止キー; status: draft/committed/revertedの状態; revision: 楽観ロック版; committed_at: 在庫へ登録した日時; reverted_at: 登録取消日時; undo_preserved_count: レシート取消時に編集・消費済みとして残した在庫件数 |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND t.user_id = %(actor_id)s`
...
| committed_at | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| expected_etag | parse_etag(if_match) (backend/src/app/core/entity_service.py:71) |
| file_sha256 | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| idempotency_key | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| reverted_at | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| revision | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/receipt_import_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
| status | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| undo_preserved_count | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `user_id = %(user_id)s; file_sha256 = %(file_sha256)s; idempotency_key = %(idempotency_key)s; status = %(status)s; revision = %(revision)s; committed_at = %(committed_at)s; reverted_at = %(reverted_at)s; undo_preserved_count = %(undo_preserved_count)s`

### `backend/src/app/apis/entities/receipt_import_update/sql/002_reference_user_id.sql`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/receipt_import_update/router.py:32 |
| execute | ReceiptImportRow.model_validate(rows[0]) | backend/src/app/apis/entities/receipt_import_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | レシート読取・在庫登録の処理単位の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/receipt_import_update/router.py:32 |
| execute | レシート読取・在庫登録の処理単位の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/receipt_import_update/functions.py:9 |
