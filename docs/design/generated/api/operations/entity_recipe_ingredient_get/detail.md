# 詳細設計: entity_recipe_ingredient_get
...
### `backend/src/app/apis/entities/recipe_ingredient_get/sql/001_get.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_ingredient | R | id: 不変の行識別子; created_at: 作成日時（UTC）; recipe_version_id: 親版; line_no: 表示順; form_id: 使用形態; product_version_id: 商品指定時の仕様版; component_id: セット内構成品を使う場合; kit_parent_line_id: 購入対象となるセットの親行; role: 料理での役割; demand_kind: 購入対象区分; amount_mode: 確定/範囲/適量; amount: 確定値または範囲下限; amount_max: 範囲上限; unit_id: 登録単位; canonical_amount: 登録版の基準量; conversion_id: 非基準単位の換算根拠; scaling_rule_id: 人数変換規則; optional: 任意追加材料; note: 材料の補足 |

対象条件: `WHERE t.id = %(row_id)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | row_id (backend/src/app/apis/entities/recipe_ingredient_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |

代入・選択式: `t.id; t.created_at; t.recipe_version_id; t.line_no; t.form_id; t.product_version_id; t.component_id; t.kit_parent_line_id; t.role; t.demand_kind; t.amount_mode; t.amount; t.amount_max; t.unit_id; t.canonical_amount; t.conversion_id; t.scaling_rule_id; t.optional; t.note; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_ingredient_get/router.py:31 |
| execute | RecipeIngredientRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_ingredient_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | レシピ材料明細の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_ingredient_get/router.py:31 |
| execute | レシピ材料明細の取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_ingredient_get/functions.py:9 |
