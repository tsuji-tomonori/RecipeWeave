# 詳細設計: entity_cooking_session_get

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`GET /api/entities/cooking_session/{row_id}` — 調理計画実行の取得
...
### `backend/src/app/apis/entities/cooking_session_get/sql/001_get.sql`
...
対象条件: `WHERE t.id = %(row_id)s AND EXISTS(SELECT owner_0.id FROM recipeweave.menu AS owner_0 WHERE owner_0.id = t.menu_id AND owner_0.user_id = %(actor_id)s)`
...
| row_id | row_id (backend/src/app/apis/entities/cooking_session_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |

代入・選択式: `t.id; t.created_at; t.menu_id; t.menu_revision; t.status; t.target_at; t.planner_version; t.input_snapshot; t.input_hash; t.current_task_index; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/cooking_session_get/router.py:31 |
| execute | CookingSessionRow.model_validate(rows[0]) | backend/src/app/apis/entities/cooking_session_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 調理計画実行の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/cooking_session_get/router.py:31 |
| execute | 調理計画実行の取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/cooking_session_get/functions.py:9 |
