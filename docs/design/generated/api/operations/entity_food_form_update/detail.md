# 詳細設計: entity_food_form_update

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`PUT /api/entities/food_form/{row_id}` — 食材形態の更新
...
### `backend/src/app/apis/entities/food_form_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.food_form | U | id: 不変の行識別子; created_at: 作成日時（UTC）; food_id: 対応食材; name: 生皮付き・冷凍刻み等; state: 処理状態; base_unit_id: 計算基準単位; quantity_basis: 数量の対象部分; status: 利用状態 |

対象条件: `WHERE t.id = %(row_id)s AND CAST(t.xmin AS TEXT) = %(expected_etag)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| base_unit_id | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
| quantity_basis | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/food_form_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |
...
代入・選択式: `food_id = %(food_id)s; name = %(name)s; state = %(state)s; base_unit_id = %(base_unit_id)s; quantity_basis = %(quantity_basis)s; status = %(status)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/food_form_update/router.py:32 |
| execute | FoodFormRow.model_validate(rows[0]) | backend/src/app/apis/entities/food_form_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 食材形態の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/food_form_update/router.py:32 |
| execute | 食材形態の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/food_form_update/functions.py:9 |
