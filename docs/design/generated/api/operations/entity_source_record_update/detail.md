# 詳細設計: entity_source_record_update
...
### `backend/src/app/apis/entities/source_record_update/sql/001_update.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.source_record | U | id: 不変の行識別子; created_at: 作成日時（UTC）; title: 根拠名; url: 公式資料URL; locator: 資料内位置; retrieved_at: 取得時点; content_hash: 参照内容のハッシュ; license_note: 利用条件・権利確認 |
...
| license_note | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| locator | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| retrieved_at | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| row_id | row_id (backend/src/app/apis/entities/source_record_update/functions.py:13) / row_id or uuid4() (backend/src/app/core/entity_service.py:65) / uuid4() (backend/src/app/core/entity_service.py:139) / uuid4() (backend/src/app/core/entity_service.py:148) / uuid4() (backend/src/app/core/entity_service.py:154) |
| title | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
| url | 検証済みリクエストモデル → payload → values → params。共有サービスがJSONB/整数列を変換する。 |
...
代入・選択式: `title = %(title)s; url = %(url)s; locator = %(locator)s; retrieved_at = %(retrieved_at)s; content_hash = %(content_hash)s; license_note = %(license_note)s`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/source_record_update/router.py:32 |
| execute | SourceRecordRow.model_validate(rows[0]) | backend/src/app/apis/entities/source_record_update/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 根拠資料の更新。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/source_record_update/router.py:32 |
| execute | 根拠資料の更新を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/source_record_update/functions.py:9 |
