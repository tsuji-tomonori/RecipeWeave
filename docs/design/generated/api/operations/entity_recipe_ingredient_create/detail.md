# 詳細設計: entity_recipe_ingredient_create

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`POST /api/entities/recipe_ingredient` — レシピ材料明細の作成
...
### `backend/src/app/apis/entities/recipe_ingredient_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_ingredient | C | id: 不変の行識別子; created_at: 作成日時（UTC）; recipe_version_id: 親版; line_no: 表示順; form_id: 使用形態; product_version_id: 商品指定時の仕様版; component_id: セット内構成品を使う場合; kit_parent_line_id: 購入対象となるセットの親行; role: 料理での役割; demand_kind: 購入対象区分; amount_mode: 確定/範囲/適量; amount: 確定値または範囲下限; amount_max: 範囲上限; unit_id: 登録単位; canonical_amount: 登録版の基準量; conversion_id: 非基準単位の換算根拠; scaling_rule_id: 人数変換規則; optional: 任意追加材料; note: 材料の補足 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_ingredient_create/router.py:31 |
| execute | RecipeIngredientRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_ingredient_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | レシピ材料明細の作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_ingredient_create/router.py:31 |
| execute | レシピ材料明細の作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_ingredient_create/functions.py:7 |
