# 詳細設計: entity_recipe_search_document_get
...
### `backend/src/app/apis/entities/recipe_search_document_get/sql/001_get.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.recipe_search_document | R | id: 不変ID; created_at: 作成日時; recipe_id: 同一性単位で1件; published_version_id: 検索対象の公開版; projection_version: 検索文書の生成器版; display_title: 表示タイトル; food_identity_ids: 検索用食品ID集合; facet_option_ids: 料理・味等の検索軸; search_text: 検索用本文; eligible: 公開可能か; source_hash: 正本一致確認; projected_at: 更新時点 |

対象条件: `WHERE t.id = %(row_id)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | row_id (backend/src/app/apis/entities/recipe_search_document_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |

代入・選択式: `t.id; t.created_at; t.recipe_id; t.published_version_id; t.projection_version; t.display_title; t.food_identity_ids; t.facet_option_ids; t.search_text; t.eligible; t.source_hash; t.projected_at; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/recipe_search_document_get/router.py:31 |
| execute | RecipeSearchDocumentRow.model_validate(rows[0]) | backend/src/app/apis/entities/recipe_search_document_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 公開検索用文書の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/recipe_search_document_get/router.py:31 |
| execute | 公開検索用文書の取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/recipe_search_document_get/functions.py:9 |
