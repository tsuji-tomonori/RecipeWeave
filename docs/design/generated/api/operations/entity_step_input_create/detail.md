# 詳細設計: entity_step_input_create

[詳細](detail.md) / [入出力](interface.md) / [ログ](messages.md) / [SQL](queries.md) / [シーケンス](sequence.md) / [要因別試験](tests.md) / [API一覧](../../README.md)

実装から自動生成。手編集禁止。`uv run python tools/generate_service_design.py` で更新。

`POST /api/entities/step_input` — 工程への材料受渡しの作成
...
### `backend/src/app/apis/entities/step_input_create/sql/001_create.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.step_input | C | id: 不変の行識別子; created_at: 作成日時（UTC）; step_id: 受取工程; material_id: 受け渡す材料; fraction: 当該ノード生成量の利用割合 |
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/step_input_create/router.py:28 |
| execute | StepInputRow.model_validate(rows[0]) | backend/src/app/apis/entities/step_input_create/functions.py:7 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 工程への材料受渡しの作成。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/step_input_create/router.py:28 |
| execute | 工程への材料受渡しの作成を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/step_input_create/functions.py:7 |
