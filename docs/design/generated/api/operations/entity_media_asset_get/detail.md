...
### `backend/src/app/apis/entities/media_asset_get/sql/001_get.sql`

実行条件: このSQLの呼出し経路で実行

| 物理テーブル | 操作 | 対象列と意味 |
|---|---|---|
| recipeweave.media_asset | R | id: 不変の行識別子; created_at: 作成日時（UTC）; operation_id: 説明する標準動作; media_type: 動画/アニメ/画像; uri: オブジェクト格納先; sha256: 資産ハッシュ; locale: 字幕言語; version: 媒体版; parameter_contract: 対応厚み・食材形状・視点; source_id: 権利・作成根拠; validation: 内容検証 |

対象条件: `WHERE t.id = %(row_id)s AND TRUE`

| SQLバインド | 実装上の値の出所 |
|---|---|
| row_id | row_id (backend/src/app/apis/entities/media_asset_get/functions.py:11) / row_id or uuid4() (backend/src/app/core/entity_service.py:66) / uuid4() (backend/src/app/core/entity_service.py:140) / uuid4() (backend/src/app/core/entity_service.py:149) / uuid4() (backend/src/app/core/entity_service.py:155) |

代入・選択式: `t.id; t.created_at; t.operation_id; t.media_type; t.uri; t.sha256; t.locale; t.version; t.parameter_contract; t.source_id; t.validation; CAST(t.xmin AS TEXT) AS etag`
...
| 関数 | 返却式 | 定義元 |
|---|---|---|
| handle | result | backend/src/app/apis/entities/media_asset_get/router.py:31 |
| execute | MediaAssetRow.model_validate(rows[0]) | backend/src/app/apis/entities/media_asset_get/functions.py:9 |
...
| 関数 | 処理 | 定義元 |
|---|---|---|
| handle | 教育用動画等の版の取得。認証情報は依存から取得し、本人所有または管理者権限を検査する。 | backend/src/app/apis/entities/media_asset_get/router.py:31 |
| execute | 教育用動画等の版の取得を固定操作契約で実行し、DB行を専用応答型へ検証する。 | backend/src/app/apis/entities/media_asset_get/functions.py:9 |
