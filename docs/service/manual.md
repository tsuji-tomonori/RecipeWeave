...
初期投入の8品は未試作の開発確認用です。試用を許可した開発環境にログインした場合だけ、「未試作の開発確認用レシピ」として表示します。Cognitoのログインを使うDevでも利用できますが、本番環境では表示を許可せず、通常の公開検索にも混ぜません。開発環境で0件なら、ログイン状態と試用の設定を管理者に確認してください。通常の公開検索で0件なら、公開料理や検索条件を確認してください。
...
人数変更後の工程に時間の確認が必要な場合は、「この料理を作る」から段取り画面へ進みます。以下の手順で、今回使う目安時間を確認してください。基準人数のままで確認が不要な一品は、そのまま調理を始められます。

1. **「この人数の目安時間を確認」を押します。**
2. 確認が必要な工程ごとに、今回の目安を分で入力します。表示された基準人数の時間は参考値で、今回の人数で実測した時間ではありません。
3. **「この目安時間で段取りを作る」を押します。**
4. **「確認した目安時間」**と作業順・器具を確認し、**「調理を始める」を押します。** 直したい場合は「目安時間を修正する」で戻れます。

入力した目安は利用者が確認した予定時間として保存します。検証済みの調理時間へ読み替えず、人数を変えたときはその人数に合う目安を改めて確認します。
...
2. 献立では使う器具と作業の順番を確認します。人数変更により工程時間の確認が必要なときは、上の手順で目安を入力して段取りを作り、**「調理を始める」を押します。**
...
本人の在庫や献立をファイルに残す場合は**「設定 → データを書き出す」を押します。** 書き出した時点へ戻す操作は、次の「バックアップから復元する」で確認できます。別端末で続きを開くだけなら、復元せず同じ利用者でログインしてください。
...
現在の版で本人が「データを書き出す」から取得したファイルを使います。読み込めるのは5MBまでの対応ファイルです。旧ブラウザ版の形式、別の利用者が書き出したファイル、内容を書き換えたファイルは使えません。

1. 書き出した利用者と同じ利用者でログインし、**「設定 → バックアップから復元」を押してファイルを選びます。**
2. **「復元する内容を確認」**で、ファイル名と書出し日時、項目ごとの現在件数・復元後の件数、置き換える対象・保持する対象、確認の有効期限を確認します。この時点では在庫や献立を置き換えません。
3. 内容が正しければ**「確認して次へ」を押します。** やめる場合はキャンセルします。
4. **「現在のデータを置き換えますか？」**を確認します。現在の内容へ追加する操作ではなく、対象の本人データをファイルの内容に戻す操作です。必要な現在データは、復元前に別ファイルへ書き出してください。
5. 戻すと決めた場合だけ**「全置換して復元」を押します。** 完了後、冷蔵庫・献立・調理状況を確認します。

置き換えるのは、本人の在庫・レシート登録履歴、献立・保存した料理・調理と消費履歴、自分で追加した食材・常備食材・除外条件・器具・買い物確認、言語・タイムゾーンです。公開レシピ・共通食品、アカウント・ログイン情報、監査やバックアップ発行記録は保持します。レシートの元画像は書き出したファイルにも含みません。

確認は15分間有効です。期限切れ、別端末での更新、参照している料理などの変更で進めなくなった場合は、同じファイルの確認画面で**「内容を再確認」を押し、最新の件数を見直します。** 自動で復元をやり直すことはありません。ファイルの形式や利用者が違う場合は設定へ戻り、正しいファイルを選び直します。

どちらの確認画面でもキャンセルすれば、在庫や献立は変わりません。復元途中の処理が失敗した場合も、一部だけを置き換えた状態にはしません。通信が途切れて結果を確認できない場合は、まず現在の内容を読み込み、完了しているか確かめてから再確認します。確認や発行の記録は、利用データの置き換えとは別に保持します。
