...
文書状態：Dev v0.1 利用者向けマニュアル。2026年9月6日現在、下記機能は実装済み・公開待ちです。公開ページでの提供開始や実際の端末での動作確認を完了したという意味ではありません。本文はこの版の操作を説明します。

## はじめに：試用版でできること

現在は公開ページの初期設定待ちで、未公開です。自動検査を通過した実装範囲と、実際に利用者が使える提供状態を分けて案内します。公開後はこの表を更新します。

| 機能 | 現在の提供状態 |
|---|---|
| 食材からの検索 | 下記8品のサンプルを実装済み・公開待ち。人数・分量は料理を開いた後に変更 |
| レシートの画像読取 | 端末内での日本語読取を実装済み・公開待ち。実ブラウザでの読取と実レシートの認識精度は未確認 |
| この端末での冷蔵庫・保存・献立 | ブラウザへの保存、データの書き出し・検証付き読込みを実装済み・公開待ち |
| 別端末への同期 | 今回の試用版では未提供。今後の機能として区別 |
| 献立・買い物・調理 | サンプル料理の人数変更、材料集計、段取り、調理再開、希望した場合の在庫減算を実装済み・公開待ち |
| 切り方ガイド・タイマー | 4種類の図と文章、終了時刻を保持するタイマーを実装済み・公開待ち。動画と画面外の音通知は未提供 |

試用版のレシピや画像には操作確認用の見本が含まれます。画像は操作の案内であり、実画面のスクリーンショットではありません。公開後も、この表と画面にある提供状態を先に確認してください。
