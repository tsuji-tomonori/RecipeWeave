...
文書状態：サービス草案。画面の名称と操作を確認するためのマニュアルです。本文は目指すサービスを説明し、試用版にない機能は提供状況で区別します。

## はじめに：試用版でできること

公開前に次の表を実際の状態に更新します。まだ確認できていない行を「利用できます」に変えません。

| 機能 | 現在の提供状態 |
|---|---|
| 食材からの検索 | 下記8品のサンプルで提供予定。公開前に検索・表示を確認 |
| レシートの画像読取 | この端末内で日本語を読み取る機能を実装予定。実画像での読取を確認してから提供済みに変更 |
| この端末での冷蔵庫・保存・献立 | この端末への保存とデータの書き出し・読込みを実装予定。再訪・復元の動作を確認してから提供済みに変更 |
| 別端末への同期 | 今回の試用版では未提供。今後の機能として区別 |
| 献立・買い物・調理 | サンプル料理で操作できるようにする予定。動作確認後に範囲を確定 |
| 切り方ガイド・タイマー | 動作する範囲を確認中 |

試用版のレシピや画像には操作確認用の見本が含まれます。公開ページにある提供状態を先に確認してください。

予定する8品は、なすと卵の醤油炒め、なすと卵のカレー炒め、トマトと卵の炒めもの、豆腐とわかめのスープ、キャベツとツナの一品、カップ焼きそばのチーズアレンジ、ツナコーンごはん、きのこのバター炒めです。任意の食材の組み合わせに候補が見つかる範囲ではなく、操作を試すための料理です。
...
食品なのに除外されていたら、**「除外した行」を開きます。** 図4では「日用品・金額などを除外しました。」と表示している部分です。対象の行を選択し、正しい食材を指定すると登録候補へ戻せます。原文にもその行がない場合は、今回の登録を終えた後に「冷蔵庫 → 手入力で追加」で補います。
...
登録後に使用・編集した食材は「確認が必要」と表示します。変更前後の内容と現在の残量を見てから確定してください。例えば今回の300gを250gへ編集していたら、表示された現在量250gが取り消し対象です。すでに使い切った・削除した分には減らす残量がありません。数量不明の行は不明の在庫項目を外し、数値の量を推測して差し引きません。
...
ガイドが読み込めない場合は図や文章で確認します。タイマーは補助機能です。画面を閉じた場合の動作は、試用版の提供状況に表示します。
